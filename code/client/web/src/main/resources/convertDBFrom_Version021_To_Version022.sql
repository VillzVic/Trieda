...
DROP PROCEDURE IF EXISTS `convertDBFrom_Version021_To_Version022` $$
CREATE DEFINER=`trieda`@`%` PROCEDURE `convertDBFrom_Version021_To_Version022`()
BEGIN
  ...
  START TRANSACTION;
    ...
    IF actual_db_version = 21 THEN
      SET msg_status = 'Base de dados compativel com script de conversao, inicio processo conversao';

      /* Adicionando descricao para salas */
      ALTER TABLE `trieda`.`salas` ADD `sal_descricao` VARCHAR(255) DEFAULT null AFTER `sal_numero`;


      INSERT INTO `trieda`.`db_version` (`db_version`) VALUES (22);
...
CALL convertDBFrom_Version021_To_Version022();