...
DROP PROCEDURE IF EXISTS `convertDBFrom_Version022_To_Version023` $$
CREATE DEFINER=`trieda`@`%` PROCEDURE `convertDBFrom_Version022_To_Version023`()
BEGIN
  ...
  START TRANSACTION;
    ...
    IF actual_db_version = 22 THEN
      ...
      ALTER TABLE `trieda`.`professores` MODIFY COLUMN `ati_id` BIGINT(20) DEFAULT NULL;


      INSERT INTO `trieda`.`db_version` (`db_version`) VALUES (23);
...
CALL convertDBFrom_Version022_To_Version023();