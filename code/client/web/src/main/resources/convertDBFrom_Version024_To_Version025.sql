...
DROP PROCEDURE IF EXISTS `convertDBFrom_Version024_To_Version025` $$
CREATE DEFINER=`trieda`@`%` PROCEDURE `convertDBFrom_Version024_To_Version025`()
BEGIN
  ...
  START TRANSACTION;
    ...
    IF actual_db_version = 24 THEN
      SET msg_status = 'Base de dados compativel com script de conversao, inicio processo conversao';

      /* TRIEDA-1776: Finalizar Módulo Completo de Geração de Demanda do Trieda */
      ALTER TABLE  `trieda`.`alunos` ADD COLUMN `aln_virtual` BIT(1) NOT NULL DEFAULT FALSE AFTER `aln_periodo`;
      ALTER TABLE  `trieda`.`alunos` ADD COLUMN `aln_criado_por_trieda` BIT(1) NOT NULL DEFAULT FALSE AFTER `aln_virtual`;

      INSERT INTO `trieda`.`db_version` (`db_version`) VALUES (25);
...
CALL convertDBFrom_Version024_To_Version025();