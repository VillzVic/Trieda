...
DROP PROCEDURE IF EXISTS `convertDBFrom_Version030_To_Version031` $$
CREATE DEFINER=`trieda`@`%` PROCEDURE `convertDBFrom_Version030_To_Version031`()
BEGIN
  ...
  START TRANSACTION;
    ...
    IF actual_db_version = 30 THEN
      ...
      RENAME TABLE `turmas_alunos` TO `turmas_alunos_demanda`;


      INSERT INTO `trieda`.`db_version` (`db_version`) VALUES (31);
...
CALL convertDBFrom_Version030_To_Version031();