...
DROP PROCEDURE IF EXISTS `convertDBFrom_Version031_To_Version031` $$
CREATE DEFINER=`trieda`@`%` PROCEDURE `convertDBFrom_Version031_To_Version032`()
BEGIN
  ...
  START TRANSACTION;
    ...
    IF actual_db_version = 31 THEN
      SET msg_status = 'Base de dados compativel com script de conversao, inicio processo conversao';

      /* TRIEDA-1918: Adicionar campo "exige equivalencia forçada" na demanda por aluno */
      ALTER TABLE  `trieda`.`alunos_demanda` ADD COLUMN `ald_exige_eq_forcada` BIT(1) DEFAULT FALSE AFTER `ald_periodo`;

      INSERT INTO `trieda`.`db_version` (`db_version`) VALUES (32);
...
CALL convertDBFrom_Version031_To_Version032();