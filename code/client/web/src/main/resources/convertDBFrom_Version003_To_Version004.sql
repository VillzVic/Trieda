...
DROP PROCEDURE IF EXISTS `convertDBFrom_Version003_To_Version004` $$
CREATE DEFINER=`trieda`@`%` PROCEDURE `convertDBFrom_Version003_To_Version004`()
BEGIN
  ...
  START TRANSACTION;
    ...
    IF actual_db_version = 3 THEN
      SET msg_status = 'Base de dados compativel com script de conversao, inicio processo conversao';
      
      /* TRIEDA-1442: Acrescentar novo parâmetro de otimização que controla se o resolvedor pode ou não utilizar demandas de prioridade 2 caso não haja total atendimento das demandas de prioridade 1 */
      ALTER TABLE `trieda`.`parametros` ADD COLUMN `par_utilizar_demandas_p2` BIT(1) DEFAULT NULL AFTER `par_regrasgendivcre`;
      UPDATE `trieda`.`parametros` SET `par_utilizar_demandas_p2` = b'1';

      INSERT INTO `trieda`.`db_version` (`db_version`) VALUES (4);
...
CALL convertDBFrom_Version003_To_Version004();