...
DROP PROCEDURE IF EXISTS `convertDBFrom_Version017_To_Version018` $$
CREATE DEFINER=`trieda`@`%` PROCEDURE `convertDBFrom_Version017_To_Version018`()
BEGIN
  ...
  START TRANSACTION;
    ...
    IF actual_db_version = 17 THEN
      ...
      ALTER TABLE `trieda`.`atendimento_operacional` ADD `atp_confirmada` BIT(1) DEFAULT false;
      ALTER TABLE `trieda`.`atendimento_tatico` ADD `att_confirmada` BIT(1) DEFAULT false;



      INSERT INTO `trieda`.`db_version` (`db_version`) VALUES (18);
...
CALL convertDBFrom_Version017_To_Version018();