...
BEGIN
  ...
  START TRANSACTION;
    ...
    IF actual_db_version = 33 THEN
      ...
      ALTER TABLE `trieda`.`alunos` ADD COLUMN `aln_prioridade` INTEGER(11) UNSIGNED NOT NULL AFTER `ins_id`;
      
      ALTER TABLE  `trieda`.`parametros` ADD COLUMN `par_consideararprioridadealunos` BIT(1) DEFAULT FALSE AFTER `par_priorizarcalouros`;
       UPDATE `trieda`.`parametros` SET `par_consideararprioridadealunos` = b'1';
       
	  ALTER TABLE  `trieda`.`parametros` ADD COLUMN `par_considerarcorequi` BIT(1) DEFAULT FALSE AFTER `par_percminint`;
	   UPDATE `trieda`.`parametros` SET `par_considerarcorequi` = b'1';
	   
	  ALTER TABLE  `trieda`.`parametros` ADD COLUMN `par_proibirgapprof` BIT(1) DEFAULT FALSE AFTER `par_mingapprof`;
	   UPDATE `trieda`.`parametros` SET `par_proibirgapprof` = b'1';
