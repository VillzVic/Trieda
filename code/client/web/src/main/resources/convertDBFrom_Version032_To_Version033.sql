...
DROP PROCEDURE IF EXISTS `convertDBFrom_Version032_To_Version033` $$
CREATE DEFINER=`trieda`@`%` PROCEDURE `convertDBFrom_Version032_To_Version033`()
BEGIN
  ...
  START TRANSACTION;
    ...
    IF actual_db_version = 32 THEN
      SET msg_status = 'Base de dados compativel com script de conversao, inicio processo conversao';

      ALTER TABLE  `trieda`.`salas` ADD COLUMN `sal_externa` BIT(1) DEFAULT FALSE AFTER `sal_custo_operacao_cred`;

      INSERT INTO `trieda`.`db_version` (`db_version`) VALUES (33);
...
CALL convertDBFrom_Version032_To_Version033();