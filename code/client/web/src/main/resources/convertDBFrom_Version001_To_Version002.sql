...
DROP PROCEDURE IF EXISTS `convertDBFrom_Version001_To_Version002` $$
CREATE DEFINER=`trieda`@`%` PROCEDURE `convertDBFrom_Version001_To_Version002`()
BEGIN
  ...
  START TRANSACTION;
    ...
    IF actual_db_version = 1 THEN
      SET msg_status = 'Base de dados compativel com script de conversao, inicio processo conversao';
      
      /* */
      ALTER TABLE `trieda`.`alunos_demanda` DROP INDEX `aln_id`, ADD UNIQUE INDEX `aln_id` USING BTREE(`aln_id`, `dem_id`, `ald_prioridade`);

      INSERT INTO `trieda`.`db_version` (`db_version`) VALUES (2);
...
CALL convertDBFrom_Version001_To_Version002();