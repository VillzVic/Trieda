...
public class GradeHorariaProfessorGrid extends ContentPanel {
	...
	protected void beforeRender() {
		...
		grid = new Grid<LinhaDeCredito>(getListStore(), new ColumnModel(
				getColumnList()));
	...
	public void requestAtendimentos() {
		...
		Services.atendimentos().getAtendimentosOperacional(getProfessorDTO(),
				getProfessorVirtualDTO(), getTurnoDTO(),
				new AsyncCallback<List<AtendimentoOperacionalDTO>>() {
					...
					public void onSuccess(List<AtendimentoOperacionalDTO> result) {
						atendimentos = result;
						preencheCores();
						grid.reconfigure(getListStore(), new ColumnModel(
								getColumnList()));
	...
	public ListStore<LinhaDeCredito> getListStore() {
		...
		if (turnoDTO != null) {
			for (Long horarioId : turnoDTO.getHorariosStringMap().keySet()) {
				store.add(new LinhaDeCredito(horarioId, turnoDTO
						.getHorariosStringMap().get(horarioId)));
			}
		}
		return store;
	}
	...
	private void addColumn(List<ColumnConfig> list, String id, String name) {
		...
		GridCellRenderer<LinhaDeCredito> change = new GridCellRenderer<LinhaDeCredito>() {
			...
			private Html content(long horarioId, int rowIndex, int colIndex) {
				...
				AtendimentoOperacionalDTO atDTO = getAtendimento(horarioId,
						semana);
				...
				final String contentToolTip = atDTO
						.getContentToolTipVisaoProfessor();
	...
	private AtendimentoOperacionalDTO getAtendimento(Long horarioId,
			Integer semana) {
