...
public class GrupoSalaAssociarSalaPresenter
	implements Presenter
{
	...
	public GrupoSalaAssociarSalaPresenter(
		Display display, SimpleGrid< GrupoSalaDTO > simpleGrid )
	{
	...
	private void populaListas()
	{
		...
		FutureSynchronizer synch = new FutureSynchronizer(
			futureSalaDTOList, futureSalaPertencesDTOList );
		...
		synch.addCallback( new AsyncCallback< Boolean >()
		{
			...
			public void onSuccess( Boolean result )
			{
				...
				ListStore< SalaDTO > storePertences
					= display.getSalasPertencesList().getStore();
	...
	private void setListeners()
	{
			...
			new SelectionListener< ButtonEvent >()
		{
			...
			{
				List< SalaDTO > salaDTOSelecionadasList
					= display.getSalasPertencesList().getStore().getModels();

				GruposSalasServiceAsync gruposSalasService = Services.gruposSalas();
				gruposSalasService.saveSalas( salaDTOSelecionadasList,
		...
		this.display.getAtualizaSalasDoAndarBT().addSelectionListener(
			new SelectionListener< ButtonEvent >()
		{
			@Override
			public void componentSelected( ButtonEvent ce )
			{
				List< SalaDTO > salaDTOAndaresList
					= display.getAndaresList().getSelectionModel().getSelectedItems();

				final List< SalaDTO > salaDTOSelecionadasList
					= display.getSalasPertencesList().getStore().getModels();
				...
				final FutureResult< ListLoadResult< SalaDTO > > futureSalaDTOList
					= new FutureResult< ListLoadResult< SalaDTO > >();

				UnidadeDTO unidadeDTO = new UnidadeDTO();
				unidadeDTO.setId( display.getGrupoSalaDTO().getUnidadeId() );
				Services.salas().getSalasDoAndareList( unidadeDTO,
					salasStringList, futureSalaDTOList );

				FutureSynchronizer synch
					= new FutureSynchronizer( futureSalaDTOList );
					...
					new AsyncCallback< Boolean >()
				{
					...
					{
						ListLoadResult< SalaDTO > salaDTOList = futureSalaDTOList.result();
						ListStore< SalaDTO > storeSalasDoAndarPertences
							= display.getSalasNaoPertencesList().getStore();
						...
						Info.display( "Atualizado",
							"Lista de salas não associadas atualizada!" );
					}
		...
		this.display.getAdicionaSalasBT().addSelectionListener(
			new SelectionListener< ButtonEvent >()
		{
			@Override
			public void componentSelected( ButtonEvent ce )
			{
				List<SalaDTO> salaDTOSelecionadasList
					= display.getSalasNaoPertencesList().getSelectionModel().getSelectedItems();

				ListStore< SalaDTO > storeSalasPertences
					= display.getSalasPertencesList().getStore();
				...
				ListStore< SalaDTO > storeSalasNaoPertences
					= display.getSalasNaoPertencesList().getStore();
				...
				Info.display( "Atualizado",
					"Salas adicionadas a lista!" );
			}
		});

		this.display.getRemoveSalasBT().addSelectionListener(
			new SelectionListener< ButtonEvent >()
		{
			@Override
			public void componentSelected( ButtonEvent ce )
	...
	{
		GrupoSalaDTO grupoSalaDTO = this.display.getGrupoSalaDTO();
		return grupoSalaDTO;
	}
