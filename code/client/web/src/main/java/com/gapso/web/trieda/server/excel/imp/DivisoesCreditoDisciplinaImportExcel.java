...
public class DivisoesCreditoDisciplinaImportExcel extends AbstractImportExcel<DivisoesCreditoDisciplinaImportExcelBean>
{
	...
	public DivisoesCreditoDisciplinaImportExcel(Cenario cenario, TriedaI18nConstants i18nConstants, TriedaI18nMessages i18nMessages, InstituicaoEnsino instituicaoEnsino)
	{
	...
	@Override
	protected DivisoesCreditoDisciplinaImportExcelBean createExcelBean(Row header, Row row)
	{
		DivisoesCreditoDisciplinaImportExcelBean bean = new DivisoesCreditoDisciplinaImportExcelBean(row.getRowNum() + 1);
	...
	@Override
	protected boolean doSyntacticValidation(List<DivisoesCreditoDisciplinaImportExcelBean> sheetContent)
	{
	...
	@Override
	protected boolean doLogicValidation(List<DivisoesCreditoDisciplinaImportExcelBean> sheetContent)
	{
	...
	private void checkNonRegisteredDisciplina(List<DivisoesCreditoDisciplinaImportExcelBean> sheetContent)
	{
	...
	private void checkUniqueness(List<DivisoesCreditoDisciplinaImportExcelBean> sheetContent)
	{
		...
		for (DivisoesCreditoDisciplinaImportExcelBean bean : sheetContent)
		{
			List<Integer> rows = divisaoCodigoToRowsMap.get(bean.getDivisaoKey());

			if (rows == null)
			{
				rows = new ArrayList<Integer>();
				divisaoCodigoToRowsMap.put(bean.getDivisaoKey(), rows);
			}
	...
	private void checkSomaCreditos(List<DivisoesCreditoDisciplinaImportExcelBean> sheetContent)
	{

		List<Integer> rowsWithError = new ArrayList<Integer>();
		for (DivisoesCreditoDisciplinaImportExcelBean bean : sheetContent)
		{
			int somaCreditos = bean.getDia1() + bean.getDia2() + bean.getDia3() + bean.getDia4() + bean.getDia5() + bean.getDia6() + bean.getDia7();
		...
		{
			getErrors().add(getI18nMessages().excelErroLogicoRegraCreditoInvalida(row.toString()));
		}
	}

	private void checkTotalCreditosDisciplina(List<DivisoesCreditoDisciplinaImportExcelBean> sheetContent)
	{
		// [ CodigoDisciplina -> Disciplina ]
		Map<String, Disciplina> disciplinaBDMap = Disciplina.buildDisciplinaCodigoToDisciplinaMap(Disciplina.findByCenario(this.instituicaoEnsino, getCenario()));
		List<Integer> rowsWithError = new ArrayList<Integer>();
		for (DivisoesCreditoDisciplinaImportExcelBean bean : sheetContent)
		{
			if (disciplinaBDMap.get(bean.getDisciplinaCodigoStr()) != null && bean.getCreditos() != disciplinaBDMap.get(bean.getDisciplinaCodigoStr()).getTotalCreditos())
			{
				rowsWithError.add(bean.getRow());
			}
		}
		...
		for (Integer row : rowsWithError)
		{
			getErrors().add(getI18nMessages().excelErroLogicoTotalCreditosDiferenteTotalDisciplina(row.toString()));
		}
	}

	@Transactional
	@Override
	protected void updateDataBase(List<DivisoesCreditoDisciplinaImportExcelBean> sheetContent)
	{
		Map<String, DivisaoCredito> divisoesBDMap = DivisaoCredito.buildDivisaoCreditoKeyToDivisaoCreditoMap(DivisaoCredito.findWithDisciplina(getCenario(), null, null, instituicaoEnsino));
