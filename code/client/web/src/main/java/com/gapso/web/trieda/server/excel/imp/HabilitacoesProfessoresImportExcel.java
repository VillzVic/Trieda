...
public class HabilitacoesProfessoresImportExcel extends AbstractImportExcel<HabilitacoesProfessoresImportExcelBean>
{
	...
	public HabilitacoesProfessoresImportExcel(Cenario cenario, TriedaI18nConstants i18nConstants, TriedaI18nMessages i18nMessages, InstituicaoEnsino instituicaoEnsino)
	{
	...
	@Override
	protected HabilitacoesProfessoresImportExcelBean createExcelBean(Row header, Row row)
	{
	...
	@Override
	protected boolean doSyntacticValidation(List<HabilitacoesProfessoresImportExcelBean> sheetContent)
	{
	...
	@Override
	protected boolean doLogicValidation(List<HabilitacoesProfessoresImportExcelBean> sheetContent)
	{
	...
	private void checkNonRegisteredProfessor(List<HabilitacoesProfessoresImportExcelBean> sheetContent)
	{
		...
		if (!rowsWithErrors.isEmpty())
		{
			getErrors().add(getI18nMessages().excelErroLogicoEntidadesNaoCadastradas(CPF_PROFESSOR_COLUMN_NAME, rowsWithErrors.toString()));
		}
	}

	@Transactional
	@Override
	protected void updateDataBase(List<HabilitacoesProfessoresImportExcelBean> sheetContent)
	{
		Map<String, ProfessorDisciplina> professoresDisciplinasBDMap = ProfessorDisciplina.buildCursoNaturalKeyToProfessorDisciplinaMap(ProfessorDisciplina.findByCenario(this.instituicaoEnsino,
						getCenario()));
		...
		for (HabilitacoesProfessoresImportExcelBean pdExcel : sheetContent)
		{
			...
			if (count == 100)
			{
				System.out.println("   Faltam " + total + " habilitações de professores");
				count = 0;
			}
		}
	}
