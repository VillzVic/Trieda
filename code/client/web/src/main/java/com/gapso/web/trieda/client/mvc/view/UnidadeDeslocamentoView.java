...
public class UnidadeDeslocamentoView extends View {

	private GTabItem tabItem;
	
	public UnidadeDeslocamentoView(Controller controller) {
		super(controller);
	}
	...
	private void onUnidadesDeslocamento() {
		tabItem = new GTabItem("Matriz de Tempos de Deslocamento entre Unidades", Resources.DEFAULTS.unidade16());
	...
	private ToolBar getToolBar() {
		...
		tb.add(bt);
		bt = new Button("", AbstractImagePrototype.create(Resources.DEFAULTS.unidade16()));
	...
	private FormPanel getFiltro() {
		...
		ComboBox<ModelData> campusCB = new ComboBox<ModelData>();
		campusCB.setStore(new ListStore<ModelData>());
		campusCB.setFieldLabel("Campus");
		form.add(campusCB, formDataFilter);
	...
	private Grid<UnidadeDeslocamentoModel> getMatriz() {
		...
		column.setRenderer(new GridCellRenderer<UnidadeDeslocamentoModel>() {
			@Override
			public Object render(UnidadeDeslocamentoModel model,
					String property, ColumnData config, int rowIndex,
					int colIndex, ListStore<UnidadeDeslocamentoModel> store,
					Grid<UnidadeDeslocamentoModel> grid) {
		...
		for(int i = 1; i<=5; i++) {
			column = new ColumnConfig("unidade"+i, "Unidade "+i, 60);
			column.setMenuDisabled(true);
			column.setSortable(false);
			...
			column.setRenderer(new GridCellRenderer<UnidadeDeslocamentoModel>() {
				@Override
				public Object render(UnidadeDeslocamentoModel model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<UnidadeDeslocamentoModel> store, Grid<UnidadeDeslocamentoModel> grid) {
		...
		ListStore<UnidadeDeslocamentoModel> store = new ListStore<UnidadeDeslocamentoModel>();  
		store.add(new UnidadeDeslocamentoModel("Unidade 1", 10.0, 20.0, 30.0, 40.0, 50.0));  
		store.add(new UnidadeDeslocamentoModel("Unidade 2", 10.0, 20.0, 30.0, 40.0, 50.0));  
		store.add(new UnidadeDeslocamentoModel("Unidade 3", 10.0, 20.0, 30.0, 40.0, 50.0));  
		store.add(new UnidadeDeslocamentoModel("Unidade 4", 10.0, 20.0, 30.0, 40.0, 50.0));  
		store.add(new UnidadeDeslocamentoModel("Unidade 5", 10.0, 20.0, 30.0, 40.0, 50.0));  
		EditorGrid<UnidadeDeslocamentoModel> grid = new EditorGrid<UnidadeDeslocamentoModel>(store, cm);
	...
	protected void handleEvent(AppEvent event) {
		...
		if (type == AppEvents.UnidadeDeslocamento) onUnidadesDeslocamento();
	}
