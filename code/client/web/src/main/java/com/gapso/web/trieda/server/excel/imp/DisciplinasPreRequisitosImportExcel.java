...
public class DisciplinasPreRequisitosImportExcel extends AbstractImportExcel<DisciplinasRequisitosImportExcelBean>
{
	...
	public DisciplinasPreRequisitosImportExcel(Cenario cenario, TriedaI18nConstants i18nConstants, TriedaI18nMessages i18nMessages, InstituicaoEnsino instituicaoEnsino)
	{
	...
	@Override
	protected DisciplinasRequisitosImportExcelBean createExcelBean(Row header, Row row)
	{
		DisciplinasRequisitosImportExcelBean bean = new DisciplinasRequisitosImportExcelBean(row.getRowNum() + 1);
	...
	@Override
	protected boolean doSyntacticValidation(List<DisciplinasRequisitosImportExcelBean> sheetContent)
	{
	...
	@Override
	protected boolean doLogicValidation(List<DisciplinasRequisitosImportExcelBean> sheetContent)
	{
	...
	private void checkUniqueness(List<DisciplinasRequisitosImportExcelBean> sheetContent)
	{
		...
		for (DisciplinasRequisitosImportExcelBean bean : sheetContent)
		{
			List<Integer> rows = disciplinaRequisitoToRowsMap.get(bean.getCurriculoStr() + "-" + bean.getPeriodoStr() + "-" + bean.getDisciplinaStr() + "-" + bean.getDisciplinaRequisitoStr());

			if (rows == null)
			{
				rows = new ArrayList<Integer>();
				disciplinaRequisitoToRowsMap.put(bean.getCurriculoStr() + "-" + bean.getPeriodoStr() + "-" + bean.getDisciplinaStr() + "-" + bean.getDisciplinaRequisitoStr(), rows);
			}
	...
	private void checkNonRegisteredDisciplina(List<DisciplinasRequisitosImportExcelBean> sheetContent)
	{
		// [ CodigoDisciplina -> Disciplina ]
		Map<String, Disciplina> disciplinaBDMap = Disciplina.buildDisciplinaCodigoToDisciplinaMap(Disciplina.findByCenario(instituicaoEnsino, getCenario()));

		List<Integer> rowsWithErrors = new ArrayList<Integer>();

		for (DisciplinasRequisitosImportExcelBean bean : sheetContent)
		{
			Disciplina disciplina = disciplinaBDMap.get(bean.getDisciplinaRequisitoStr());
		...
		{
			getErrors().add(getI18nMessages().excelErroLogicoEntidadesNaoCadastradas(DISCIPLINA_REQUISITO_COLUMN_NAME, rowsWithErrors.toString()));
		}
	}

	private void checkNonRegisteredCurriculoDisciplina(List<DisciplinasRequisitosImportExcelBean> sheetContent)
	{
		// [ CurriculoDisciplinaNaturalKey -> CurriculoDisciplina ]
		Map<String, CurriculoDisciplina> curriculoDisciplinaBDMap = CurriculoDisciplina.buildNaturalKeySemCursoToCurriculoDisciplinaMap(CurriculoDisciplina.findByCenario(instituicaoEnsino,
						getCenario()));
		...
		for (DisciplinasRequisitosImportExcelBean bean : sheetContent)
		{
			CurriculoDisciplina curriculoDisciplina = curriculoDisciplinaBDMap.get(bean.getCurriculoStr() + "-" + bean.getPeriodo().toString() + "-" + bean.getDisciplinaStr());
	...
	@Override
	protected void updateDataBase(List<DisciplinasRequisitosImportExcelBean> sheetContent)
	{
		// [ CurriculoDisciplinaNaturalKey -> CurriculoDisciplina ]
		Map<String, CurriculoDisciplina> curriculoDisciplinaBDMap = CurriculoDisciplina.buildNaturalKeySemCursoToCurriculoDisciplinaMap(CurriculoDisciplina.findByCenario(instituicaoEnsino,
						getCenario()));
		...
		{
			CurriculoDisciplina curriculoDisciplina = curriculoDisciplinaBDMap.get(disciplinaExcel.getCurriculoStr() + "-" + disciplinaExcel.getPeriodo().toString() + "-"
							+ disciplinaExcel.getDisciplinaStr());

			if (curriculoDisciplina != null)
			{
				// Update
				curriculoDisciplina.getPreRequisitos().add(disciplinaExcel.getDisciplinaRequisito());
			}
		}
	}
	...
	private void resolveHeaderColumnNames()
	{
		if (CURRICULO_COLUMN_NAME == null)
		{
			...
			DISCIPLINA_REQUISITO_COLUMN_NAME = HtmlUtils.htmlUnescape(getI18nConstants().codigoDisciplinaPreRequisito());
		}
	}
}
