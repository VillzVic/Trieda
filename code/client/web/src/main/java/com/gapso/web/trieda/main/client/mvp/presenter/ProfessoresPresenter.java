	...
	private void configureProxy() {
		...
		RpcProxy<PagingLoadResult<ProfessorDTO>> proxy = new RpcProxy<PagingLoadResult<ProfessorDTO>>() {
			...
			public void load(Object loadConfig,
				...
				TipoContratoDTO tipoContratoDTO = display
						.getTipoContratoBuscaComboBox().getValue();
				TitulacaoDTO titulacaoDTO = display.getTitulacaoBuscaComboBox()
						.getValue();
				AreaTitulacaoDTO areaTitulacaoDTO = display
						.getAreaTitulacaoBuscaComboBox().getValue();
				service.getBuscaList(cpf, tipoContratoDTO, titulacaoDTO,
						areaTitulacaoDTO, (PagingLoadConfig) loadConfig,
						callback);
			}
		};
	...
	private void setListeners() {
		...
		display.getEditButton().addSelectionListener(
				new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						final ProfessorDTO professorDTO = display.getGrid()
								.getGrid().getSelectionModel()
								.getSelectedItem();

						final ProfessoresServiceAsync professoresService = Services
								.professores();
						final AreasTitulacaoServiceAsync areasTitulacaoService = Services
								.areasTitulacao();
						...
						professoresService.getTipoContrato(
								professorDTO.getTipoContratoId(),
								futureTipoContratoDTO);
						professoresService.getTitulacao(
								professorDTO.getTitulacaoId(),
								futureTitulacaoDTO);
						areasTitulacaoService.getAreaTitulacao(
								professorDTO.getAreaTitulacaoId(),
								futureAreaTitulacaoDTO);

						FutureSynchronizer synch = new FutureSynchronizer(
								futureTipoContratoDTO, futureTitulacaoDTO,
								futureAreaTitulacaoDTO);
		...
		display.getRemoveButton().addSelectionListener(
				new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						List<ProfessorDTO> list = display.getGrid().getGrid()
								.getSelectionModel().getSelectedItems();
						final ProfessoresServiceAsync service = Services
								.professores();
