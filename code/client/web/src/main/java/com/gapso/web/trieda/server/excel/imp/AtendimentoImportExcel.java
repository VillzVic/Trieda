...
public class AtendimentoImportExcel extends AbstractImportExcel< AtendimentoImportExcelBean > {	
	...
	protected boolean sheetMustBeProcessed(int sheetIndex, Sheet sheet, Workbook workbook ) {
		...
		return ExcelInformationType.ATENDIMENTOS_POR_ALUNO.getSheetName().equals( sheetName );
	}
	...
	@Override
	protected AtendimentoImportExcelBean createExcelBean(Row header, Row row, int sheetIndex, Sheet sheet, Workbook workbook ) {
	...
	protected void processSheetContent(String sheetName, List< AtendimentoImportExcelBean > sheetContent ) {
	...
	private boolean doSyntacticValidation(String sheetName, List< AtendimentoImportExcelBean > sheetContent ) {
	...
	private boolean doLogicValidation(String sheetName, List< AtendimentoImportExcelBean > sheetContent ) {
	...
	protected void updateDataBase(String sheetName, List<AtendimentoImportExcelBean> sheetContent) {
		...
		for ( AtendimentoImportExcelBean bean : sheetContent ) {
			...
			count++;total--;if (count == 100) {System.out.println("   Faltam "+total+" Atendimentos"); count = 0;}
		}
	...
	private HorarioDisponivelCenario getHDC(SemanaLetiva semLet, Semanas diaSem, Turno turno, String hi, Map<SemanaLetiva, Map<Semanas, Map<Turno, Map<String, HorarioDisponivelCenario>>>> semLetToDiaSemToTurnoToHIToHDCMap) {
	...
	private AtendimentoOperacional criaAtendimentoOp(String turma, Sala sala, Oferta oferta, Disciplina disciplina, Disciplina disciplinaSubstituta, Professor professor, Boolean creditoTeorico, HorarioDisponivelCenario hdc) {
