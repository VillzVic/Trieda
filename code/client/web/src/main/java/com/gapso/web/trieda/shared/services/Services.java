...
public class Services
{
	...
	{
		AreasTitulacaoServiceAsync service
			= (AreasTitulacaoServiceAsync) Registry.get( AREASTITULACAO );
		if ( service == null )
		{
	...
	{
		SemanasLetivaServiceAsync service
			= (SemanasLetivaServiceAsync) Registry.get( SEMANASLETIVA );
		if ( service == null )
		{
	...
	{
		DivisoesCreditosServiceAsync service
			= (DivisoesCreditosServiceAsync) Registry.get( DIVISAOCREDITOS );
		if ( service == null )
		{
	...
	{
		ProfessoresDisciplinaServiceAsync service
			= (ProfessoresDisciplinaServiceAsync) Registry.get( PROFESSORESDISCIPLINA );
		if ( service == null )
		{
	...
	{
		UniquesServiceAsync service = (UniquesServiceAsync) Registry.get( UNIQUES );
		if ( service == null )
		{
	...
	public static DisponibilidadesServiceAsync disponibilidades()
	{
		DisponibilidadesServiceAsync service = (DisponibilidadesServiceAsync) Registry.get( DISPONIBILIDADES );
		if ( service == null )
		{
			service = GWT.create( DisponibilidadesService.class );
			Registry.register( DISPONIBILIDADES, service );
		}
