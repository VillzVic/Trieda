...
import com.gapso.web.trieda.main.client.mvp.view.DisciplinaCoRequisitoFormView;
...
public class DisciplinasCoRequisitosPresenter
implements Presenter
{
		...
		extends ITriedaI18nGateway
	{
		...
		SimpleGrid< DisciplinaRequisitoDTO > getGrid();
		Component getComponent();
		void setProxy( RpcProxy< PagingLoadResult< DisciplinaRequisitoDTO > > proxy );
	}
	...
	public DisciplinasCoRequisitosPresenter(
	...
	private void configureProxy()
	{
		...
		RpcProxy< PagingLoadResult< DisciplinaRequisitoDTO > > proxy =
			new RpcProxy< PagingLoadResult< DisciplinaRequisitoDTO > >()
		{
			@Override
			public void load( Object loadConfig,
				AsyncCallback< PagingLoadResult< DisciplinaRequisitoDTO > > callback )
			{
				...
				service.getDisciplinasCoRequisitosList( cenario, disciplinaDTO, curriculoDTO, periodo,
	...
	private void setListeners()
	{
			...
			new SelectionListener< ButtonEvent >()
		{
			...
			{
				Presenter presenter = new DisciplinaCoRequisitoFormPresenter( instituicaoEnsinoDTO, cenario,
					new DisciplinaCoRequisitoFormView( cenario ),  display.getGrid());
			...
			new SelectionListener< ButtonEvent >()
		{
			...
			public void componentSelected( ButtonEvent ce )
			{
				List< DisciplinaRequisitoDTO > list
					= display.getGrid().getGrid().getSelectionModel().getSelectedItems();

				final CurriculosServiceAsync service = Services.curriculos();

				service.removeDisciplinasCoRequisitos( cenario, list, new AsyncCallback< Void >()
				{
					@Override
					public void onFailure( Throwable caught )
					{
						MessageBox.alert( "ERRO!", "Não foi possível remover o(s) co-requisito(s)", null );
					}
			...
			new SelectionListener< ButtonEvent >()
		{
			...
			public void componentSelected( ButtonEvent ce )
			{
				ExcelParametros parametros = new ExcelParametros(
					ExcelInformationType.DISCIPLINAS_CO_REQUISITOS, instituicaoEnsinoDTO, cenario );
			...
			new SelectionListener< MenuEvent >()
		{
			...
			public void componentSelected( MenuEvent ce )
			{
				...
				ExcelParametros parametros = new ExcelParametros(
					ExcelInformationType.DISCIPLINAS_CO_REQUISITOS, instituicaoEnsinoDTO, cenario, fileExtension );
