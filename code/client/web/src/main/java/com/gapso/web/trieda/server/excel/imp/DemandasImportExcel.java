...
public class DemandasImportExcel extends AbstractImportExcel<DemandasImportExcelBean> {
	...
	public DemandasImportExcel(Cenario cenario, TriedaI18nConstants i18nConstants, TriedaI18nMessages i18nMessages) {
	...
	@Override
	protected DemandasImportExcelBean createExcelBean(HSSFRow header, HSSFRow row, int sheetIndex, HSSFSheet sheet, HSSFWorkbook workbook) {
	...
	private boolean doSyntacticValidation(String sheetName, List<DemandasImportExcelBean> sheetContent) {
	...
	private boolean doLogicValidation(String sheetName, List<DemandasImportExcelBean> sheetContent) {
		// verifica se uma demanda (campus + turno + matrizcurricular + disciplina) é adicionada mais de uma vez no arquivo de entrada
	...
	private void checkUniqueness(List<DemandasImportExcelBean> sheetContent) {
		// map com os códigos "campus + turno + matrizcurricular + periodo + disciplina" e as linhas em que o mesmo aparece no arquivo de entrada
		// ["campus + turno + matrizcurricular + periodo + disciplina" -> Lista de Linhas do Arquivo de Entrada]
	...
	private void updateDataBase(String sheetName, List<DemandasImportExcelBean> sheetContent) {
		Map<String,Demanda> demandasBDMap = Demanda.buildCampusTurnoCurriculoDisciplinaToDemandaMap(Demanda.findAll());
		Map<String,Oferta> ofertasBDMap = Oferta.buildCampusTurnoCurriculoToOfertaMap(Oferta.findByCenario(getCenario()));
		
		for (DemandasImportExcelBean demandasExcel : sheetContent) {
			Demanda demandaBD = demandasBDMap.get(getCodeDemanda(demandasExcel));
			
			String codigoOferta = demandasExcel.getCampusStr() + "-" + demandasExcel.getTurnoStr() + "-" + demandasExcel.getMatrizCurricularStr();
			Oferta oferta = ofertasBDMap.get(codigoOferta);
