...
public class DisciplinasSalasExportExcel extends AbstractExportExcel {
	...
	private int writeData(Sala sala, int row, HSSFSheet sheet) {
		...
		for (CurriculoDisciplina disciplinaDeUmPeriodo : sala
		...
		for (Integer periodo : periodoToCurriculosDisciplinasMap.keySet()) {
			for (CurriculoDisciplina disciplinaDeUmPeriodo : periodoToCurriculosDisciplinasMap
					.get(periodo)) {
				// Sala
				setCell(row, 2, sheet,
				...
				setCell(row, 3, sheet,
						cellStyles[ExcelCellStyleReference.TEXT.ordinal()],
						disciplinaDeUmPeriodo.getCurriculo().getCurso()
								.getCodigo());
				// Matriz Curricular
				setCell(row, 4, sheet,
						cellStyles[ExcelCellStyleReference.TEXT.ordinal()],
