...
public class AlunosDemandaImportExcel extends AbstractImportExcel<AlunosDemandaImportExcelBean>
{
	...
	public AlunosDemandaImportExcel(Cenario cenario, TriedaI18nConstants i18nConstants, TriedaI18nMessages i18nMessages, InstituicaoEnsino instituicaoEnsino)
	{
	...
	@Override
	protected AlunosDemandaImportExcelBean createExcelBean(Row header, Row row)
	{
		AlunosDemandaImportExcelBean bean = new AlunosDemandaImportExcelBean(row.getRowNum() + 1);
	...
	@Override
	protected boolean doSyntacticValidation(List<AlunosDemandaImportExcelBean> sheetContent)
	{
	...
	@Override
	protected boolean doLogicValidation(List<AlunosDemandaImportExcelBean> sheetContent)
	{
	...
	private void checkNonRegisteredCampus(List<AlunosDemandaImportExcelBean> sheetContent)
	{
		// [ CódidoCampus -> Campus ]
	...
	private void checkNonRegisteredTurno(List<AlunosDemandaImportExcelBean> sheetContent)
	{
		// [ NomeTurno -> Turno ]
	...
	private void checkNonRegisteredCurso(List<AlunosDemandaImportExcelBean> sheetContent)
	{
		// [ CódigoCurso -> Curso ]
	...
	private void checkNonRegisteredDisciplina(List<AlunosDemandaImportExcelBean> sheetContent)
	{
	...
	private void checkNonRegisteredCurriculo(List<AlunosDemandaImportExcelBean> sheetContent)
	{
	...
	protected void updateDataBase(List<AlunosDemandaImportExcelBean> sheetContent)
	{
		...
		for (AlunosDemandaImportExcelBean alunosDemandaExcel : sheetContent)
		{
			...
			if (demandaBD != null && alunoBD != null)
			{
				...
				String codCurriculoDisciplina = getCodeCurriculoDisciplina(alunosDemandaExcel);
			...
			if (count == 100)
			{
				System.out.println("   Faltam " + total + " AlunosDemanda");
				count = 0;
			}
		}
	...
	private void atualizaQuantidadeDemanda()
	{
		...
		Map<Demanda, Set<String>> map = new HashMap<Demanda, Set<String>>();
		...
		for (Entry<Demanda, Set<String>> entry : map.entrySet())
		{
			...
			if (count == 100)
			{
				System.out.println("   100 Demandas processados");
				count = 0;
			}
		}
	}
	...
	{
		return bean.getCodigoCampusStr() + "-" + bean.getCodigoTurnoStr() + "-" + bean.getCodigoCurriculoStr() + "-" + bean.getDisciplinaCodigoStr();
	}
	...
	{
		return bean.getCodigoCampusStr() + "-" + bean.getCodigoTurnoStr() + "-" + bean.getCodigoCurriculoStr();
	}
	...
	private String getCodeCurriculoDisciplina(AlunosDemandaImportExcelBean bean)
	{
		String key = "";
