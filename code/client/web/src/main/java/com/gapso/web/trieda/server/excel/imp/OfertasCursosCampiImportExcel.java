...
public class OfertasCursosCampiImportExcel extends
	...
	protected OfertasCursosCampiImportExcel(Cenario cenario,
	...
	@Override
	protected boolean sheetMustBeProcessed(int sheetIndex, Sheet sheet,
			Workbook workbook) {
		String sheetName = workbook.getSheetName(sheetIndex);
		return ExcelInformationType.OFERTAS_CURSOS_CAMPI.getSheetName().equals(
				sheetName);
	}
	...
	@Override
	protected OfertasCursosCampiImportExcelBean createExcelBean(Row header,
			Row row, int sheetIndex, Sheet sheet, Workbook workbook) {
		...
		for (int cellIndex = row.getFirstCellNum(); cellIndex <= row
			...
			if (cell != null) {
				...
				if (headerCell != null) {
					String columnName = headerCell.getRichStringCellValue()
							.getString();
	...
	private boolean doSyntacticValidation(String sheetName,
		...
		for (ImportExcelError error : syntacticErrorsMap.keySet()) {
			...
			getErrors().add(
					error.getMessage(linhasComErro.toString(),
							getI18nMessages()));
		}
	...
	private void updateDataBase(String sheetName,
		...
		for (OfertasCursosCampiImportExcelBean demandasExcel : sheetContent) {
			...
			if (oferta == null) {
				...
				oferta.setReceitaWithPrecision(demandasExcel.getReceita() == null ? 0.0
						: demandasExcel.getReceita());
	...
	private void checkUniqueness(
		...
		for (Entry<String, List<Integer>> entry : ofestasToRowsMap.entrySet()) {
			if (entry.getValue().size() > 1) {
				getErrors().add(
						getI18nMessages().excelErroLogicoUnicidadeViolada(
	...
	private void checkNonRegisteredCampus(
			List<OfertasCursosCampiImportExcelBean> sheetContent) {
		// [ CódidoCampus -> Campus ]
		Map<String, Campus> campiBDMap = Campus
				.buildCampusCodigoToCampusMap(Campus.findByCenario(
						this.instituicaoEnsino, getCenario()));
		...
		if (!rowsWithErrors.isEmpty()) {
			getErrors().add(
					getI18nMessages().excelErroLogicoEntidadesNaoCadastradas(
	...
	private void checkNonRegisteredTurno(
			List<OfertasCursosCampiImportExcelBean> sheetContent) {
		// [ NomeTurno -> Turno ]
		Map<String, Turno> turnosBDMap = Turno.buildTurnoNomeToTurnoMap(Turno
				.findAll(this.instituicaoEnsino, getCenario()));
	...
	private void checkNonRegisteredCurso(
			List<OfertasCursosCampiImportExcelBean> sheetContent) {
		// [ CódigoCurso -> Curso ]
		Map<String, Curso> cursosBDMap = Curso.buildCursoCodigoToCursoMap(Curso
				.findByCenario(this.instituicaoEnsino, getCenario()));
	...
	private void checkNonRegisteredCurriculo(
		...
		Map<String, Curriculo> curriculosBDMap = Curriculo
				.buildCurriculoCodigoToCurriculoMap(Curriculo.findByCenario(
						this.instituicaoEnsino, getCenario()));

		List<Integer> rowsWithErrors = new ArrayList<Integer>();

		for (OfertasCursosCampiImportExcelBean bean : sheetContent) {
			Curriculo curriculo = curriculosBDMap.get(bean
					.getMatrizCurricularStr());
	...
	private String getCodeOferta(OfertasCursosCampiImportExcelBean bean) {
		return bean.getCampusStr() + "-" + bean.getTurnoStr() + "-"
				+ bean.getMatrizCurricularStr();
	}

	private void resolveHeaderColumnNames() {
		if (CAMPUS_COLUMN_NAME == null) {
			CAMPUS_COLUMN_NAME = HtmlUtils.htmlUnescape(getI18nConstants()
					.codigoCampus());
			TURNO_COLUMN_NAME = HtmlUtils.htmlUnescape(getI18nConstants()
					.turno());
			CURSO_COLUMN_NAME = HtmlUtils.htmlUnescape(getI18nConstants()
					.codigoCurso());
			MATRIZ_CURRICULAR_COLUMN_NAME = HtmlUtils
					.htmlUnescape(getI18nConstants().matrizCurricular());
			RECEITA_COLUMN_NAME = HtmlUtils
					.htmlUnescape("Receita por Crédito (R$)");
		}
	}
