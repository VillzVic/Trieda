...
public class EquivalenciasImportExcel extends AbstractImportExcel<EquivalenciasImportExcelBean>
{
	...
	public EquivalenciasImportExcel(Cenario cenario, TriedaI18nConstants i18nConstants, TriedaI18nMessages i18nMessages, InstituicaoEnsino instituicaoEnsino)
	{
	...
	@Override
	protected EquivalenciasImportExcelBean createExcelBean(Row header, Row row)
	{
	...
	@Override
	protected boolean doSyntacticValidation(List<EquivalenciasImportExcelBean> sheetContent)
	{
	...
	@Override
	protected boolean doLogicValidation(List<EquivalenciasImportExcelBean> sheetContent)
	{
	...
	private void checkEquivalenciaGeral(List<EquivalenciasImportExcelBean> sheetContent)
	{
		// [ Equivalencia -> Equivalencia Geral ]
		Map<String, Boolean> codigoToRowsMap = new HashMap<String, Boolean>();
	...
	private void checkNonRegisteredDisciplina(List<EquivalenciasImportExcelBean> sheetContent)
	{
		...
		Map<String, Disciplina> disciplinaBDMap = Disciplina.buildDisciplinaCodigoToDisciplinaMap(Disciplina.findByCenario(this.instituicaoEnsino, getCenario()));

		Map<String, Curso> cursoBDMap = Curso.buildCursoCodigoToCursoMap(Curso.findByCenario(this.instituicaoEnsino, getCenario()));
		...
		for (EquivalenciasImportExcelBean bean : sheetContent)
		{
			Disciplina disciplinaCursou = disciplinaBDMap.get(bean.getCursouStr());

			Disciplina disciplinaElimina = disciplinaBDMap.get(bean.getEliminaStr());

			Curso equivalenciaCurso = cursoBDMap.get(bean.getCursoStr());
		...
		{
			getErrors().add(getI18nMessages().excelErroLogicoEntidadesNaoCadastradas(CURSO_COLUMN_NAME, rowsWithErrorsElimina.toString()));
		}

		/*
		 * for ( EquivalenciasImportExcelBean bean : sheetContent ) { String [] eliminaList = bean.getEliminaStr().split( ";" );
		 * 
		 * for ( String eliminaString : eliminaList ) { Disciplina disciplina = disciplinaBDMap.get( eliminaString.trim() );
		 * 
		 * if ( disciplina != null ) { bean.getDisciplinasElimina().add( disciplina ); } else { rowsWithErrorsCursou.add( bean.getRow() ); } } }
		 */

		/*
		 * if ( !rowsWithErrorsCursou.isEmpty() ) { getErrors().add( getI18nMessages().excelErroLogicoEntidadesNaoCadastradas( CURSOU_COLUMN_NAME, rowsWithErrorsCursou.toString() )
		 * ); }
		 * 
		 * if ( !rowsWithErrorsElimina.isEmpty() ) { getErrors().add( getI18nMessages().excelErroLogicoEntidadesNaoCadastradas( ELIMINA_COLUMN_NAME,
		 * rowsWithErrorsElimina.toString() ) ); }
		 */
	}

	@Transactional
	@Override
	protected void updateDataBase(List<EquivalenciasImportExcelBean> sheetContent)
	{
		Map<String, Equivalencia> equivalenciasBDMap = Equivalencia.buildEquivalenciaCursouCodigoEliminaCodigoToEquivalenciaMap(Equivalencia.findByCenario(this.instituicaoEnsino, getCenario()));
		...
		{
			if (equivalenciaExcel.getDisciplinaCursou() == null || equivalenciaExcel.getDisciplinaElimina() == null)
			{
				continue;
			}

			Equivalencia equivalenciaBD = equivalenciasBDMap.get(equivalenciaExcel.getCursouStr() + equivalenciaExcel.getEliminaStr());
