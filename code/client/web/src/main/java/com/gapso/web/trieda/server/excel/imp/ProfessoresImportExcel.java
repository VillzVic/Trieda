	...
	extends AbstractImportExcel< ProfessoresImportExcelBean >
{
	...
	@Override
	protected ProfessoresImportExcelBean createExcelBean(
		HSSFRow header, HSSFRow row, int sheetIndex,
		HSSFSheet sheet, HSSFWorkbook workbook )
	{
		ProfessoresImportExcelBean bean
			= new ProfessoresImportExcelBean( row.getRowNum() + 1 );
		...
		List< ProfessoresImportExcelBean > sheetContent )
	{
		...
		Map< String, List< Integer > > professorCpfToRowsMap
		...
		for ( Entry< String, List< Integer > > entry
			: professorCpfToRowsMap.entrySet() )
		{
		...
		List< ProfessoresImportExcelBean > sheetContent )
	{
		...
		{
			getErrors().add( getI18nMessages().excelErroLogicoEntidadesNaoCadastradas(
				TITULACAO_COLUMN_NAME, rowsWithErrors.toString() ) );
		}
	}
