...
public class AreasTitulacaoImportExcel extends AbstractImportExcel<AreasTitulacaoImportExcelBean> {
	...
	public AreasTitulacaoImportExcel(Cenario cenario, TriedaI18nConstants i18nConstants, TriedaI18nMessages i18nMessages) {
	...
	@Override
	protected AreasTitulacaoImportExcelBean createExcelBean(HSSFRow header, HSSFRow row, int sheetIndex, HSSFSheet sheet, HSSFWorkbook workbook) {
	...
	@Override
	protected void processSheetContent(String sheetName, List<AreasTitulacaoImportExcelBean> sheetContent) {
		if (doSyntacticValidation(sheetName,sheetContent) && doLogicValidation(sheetName,sheetContent)) {
			updateDataBase(sheetName,sheetContent);
		}
	}

	private boolean doSyntacticValidation(String sheetName, List<AreasTitulacaoImportExcelBean> sheetContent) {
	...
	private boolean doLogicValidation(String sheetName, List<AreasTitulacaoImportExcelBean> sheetContent) {
		// verifica se alguma area apareceu mais de uma vez no arquivo de entrada
	...
	private void checkUniqueness(List<AreasTitulacaoImportExcelBean> sheetContent) {
		// map com os códigos das areas e as linhas em que a mesma aparece no arquivo de entrada
