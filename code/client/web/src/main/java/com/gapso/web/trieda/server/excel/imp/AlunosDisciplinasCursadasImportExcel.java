...
public class AlunosDisciplinasCursadasImportExcel
	extends AbstractImportExcel< AlunosDisciplinasCursadasImportExcelBean >
{
	...
	public AlunosDisciplinasCursadasImportExcel( Cenario cenario,
		...
		int sheetIndex, Sheet sheet, Workbook workbook )
	{
		...
		return ExcelInformationType.ALUNOS_DISCIPLINAS_CURSADAS.getSheetName().equals( sheetName );
	}
	...
	@Override
	protected AlunosDisciplinasCursadasImportExcelBean createExcelBean(
		Row header, Row row, int sheetIndex,
		Sheet sheet, Workbook workbook )
	{
		AlunosDisciplinasCursadasImportExcelBean bean
			= new AlunosDisciplinasCursadasImportExcelBean( row.getRowNum() + 1 );
	...
	protected void processSheetContent(
		String sheetName, List< AlunosDisciplinasCursadasImportExcelBean > sheetContent )
	{
	...
	private void checkUniqueness(
		List< AlunosDisciplinasCursadasImportExcelBean > sheetContent )
	{
		...
		for ( AlunosDisciplinasCursadasImportExcelBean bean : sheetContent )
		{
			List< Integer > rows = disciplinaRequisitoToRowsMap.get( bean.getCursoStr() + "-" + bean.getCurriculoStr()
					+ "-" + bean.getPeriodoStr() + "-" + bean.getDisciplinaStr() + "-" + bean.getAlunoStr() );
	
			if ( rows == null )
			{
				rows = new ArrayList< Integer >();
				disciplinaRequisitoToRowsMap.put( bean.getCursoStr() + "-" + bean.getCurriculoStr() + "-" + bean.getPeriodoStr()
						+ "-" +	bean.getDisciplinaStr() + "-" +	bean.getAlunoStr(), rows );
			}
	...
	private void checkNonRegisteredAluno(
		List< AlunosDisciplinasCursadasImportExcelBean > sheetContent )
	{
		// [ Matricula -> Aluno ]
		Map< String, Aluno > alunoBDMap
			= Aluno.buildMatriculaAlunoToAlunoMap(
					Aluno.findByCenario(instituicaoEnsino, getCenario()) );
		...
		{
			Aluno aluno
				= alunoBDMap.get( bean.getAlunoStr() );
		...
		if ( !rowsWithErrors.isEmpty() )
		{
			getErrors().add( getI18nMessages().excelErroLogicoEntidadesNaoCadastradas(
				ALUNO_COLUMN_NOME, rowsWithErrors.toString() ) );
		}
	}
		...
		List< AlunosDisciplinasCursadasImportExcelBean > sheetContent )
	{
		...
		for ( AlunosDisciplinasCursadasImportExcelBean bean : sheetContent )
		{
			CurriculoDisciplina curriculoDisciplina
				= curriculoDisciplinaBDMap.get( bean.getCursoStr() + "-" + bean.getCurriculoStr() + "-" +
		...
		String sheetName, List< AlunosDisciplinasCursadasImportExcelBean > sheetContent )
	{
		...
		{
			CurriculoDisciplina curriculoDisciplina
			= curriculoDisciplinaBDMap.get( disciplinaExcel.getCursoStr() + "-" +disciplinaExcel.getCurriculoStr() + "-" +
