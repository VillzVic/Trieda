...
public class CenariosServiceImpl extends RemoteService implements CenariosService, ProgressDeclaration
{
	...
	/*
	 * public PagingLoadResult< CenarioDTO > getBuscaList( Integer ano, Integer semestre, PagingLoadConfig config ) { List< CenarioDTO > list = new ArrayList< CenarioDTO >();
	 * String orderBy = config.getSortField();
	 * 
	 * if ( orderBy != null ) { if ( config.getSortDir() != null && config.getSortDir().equals( SortDir.DESC ) ) { orderBy = ( orderBy + " asc" ); } else { orderBy = ( orderBy +
	 * " desc" ); } }
	 * 
	 * List< Cenario > listCenarios = Cenario.findByAnoAndSemestre( this.getInstituicaoEnsinoSuperUser(), ano, semestre, config.getOffset(), config.getLimit(), orderBy );
	 * 
	 * for ( Cenario cenario : listCenarios ) { if (isRoleAdministrator()) list.add( ConvertBeans.toCenarioDTO( cenario ) ); else if
	 * (getUsuario().getUsername().equals(cenario.getCriadoPor().getUsername())) list.add( ConvertBeans.toCenarioDTO( cenario ) ); }
	 * 
	 * BasePagingLoadResult< CenarioDTO > result = new BasePagingLoadResult< CenarioDTO >( list );
	 * 
	 * result.setOffset( config.getOffset() ); result.setTotalLength( Cenario.count( this.getInstituicaoEnsinoUser() ) );
	 * 
	 * return result; }
	 */
	...
	/*
	 * @Override public void removeAllAlunos(CenarioDTO cenarioDTO) { Cenario cenario = Cenario.find(cenarioDTO.getId(), getInstituicaoEnsinoUser());
	 * Cenario.removeAllAlunos(cenario); }
	 * 
	 * @Override public void removeAllAlunosDemanda(CenarioDTO cenarioDTO) { Cenario cenario = Cenario.find(cenarioDTO.getId(), getInstituicaoEnsinoUser());
	 * Cenario.removeAllAlunos(cenario); }
	 * 
	 * @Override public void removeAllEquivalencias(CenarioDTO cenarioDTO) { Cenario cenario = Cenario.find(cenarioDTO.getId(), getInstituicaoEnsinoUser());
	 * Cenario.removeAllAlunos(cenario); }
	 */
