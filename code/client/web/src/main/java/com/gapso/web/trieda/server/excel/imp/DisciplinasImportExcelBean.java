...
public class DisciplinasImportExcelBean extends AbstractImportExcelBean implements Comparable<DisciplinasImportExcelBean> {
	...
	public List<ImportExcelError> checkSyntacticErrors() {
		...
		if (!tudoVazio()) {
			...
			creditosTeoricos = checkNonNegativeIntegerField(creditosTeoricosStr,ImportExcelError.DISCIPLINA_CREDITOS_TEORICOS_FORMATO_INVALIDO,ImportExcelError.DISCIPLINA_CREDITOS_TEORICOS_VALOR_NEGATIVO,erros);
			creditosPraticos = checkNonNegativeIntegerField(creditosPraticosStr,ImportExcelError.DISCIPLINA_CREDITOS_PRATICOS_FORMATO_INVALIDO,ImportExcelError.DISCIPLINA_CREDITOS_PRATICOS_VALOR_NEGATIVO,erros);
			usaLaboratorio = checkBooleanField(usaLaboratorioStr,ImportExcelError.DISCIPLINA_USA_LABORATORIO_FORMATO_INVALIDO,erros);
			nivelDificuldade = checkEnumField(nivelDificuldadeStr,Dificuldades.class,ImportExcelError.DISCIPLINA_NIVEL_DIFICULDADE_VALOR_INVALIDO,erros);
			maxAlunosTeorico = checkNonNegativeIntegerField(maxAlunosTeoricoStr,ImportExcelError.DISCIPLINA_MAX_ALUNOS_TEORICOS_FORMATO_INVALIDO,ImportExcelError.DISCIPLINA_MAX_ALUNOS_TEORICOS_VALOR_NEGATIVO,erros);
			maxAlunosPratico = checkNonNegativeIntegerField(maxAlunosPraticoStr,ImportExcelError.DISCIPLINA_MAX_ALUNOS_PRATICOS_FORMATO_INVALIDO,ImportExcelError.DISCIPLINA_MAX_ALUNOS_PRATICOS_VALOR_NEGATIVO,erros);
		} else {
