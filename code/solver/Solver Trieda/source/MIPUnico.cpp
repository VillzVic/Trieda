...
const bool CONSTR_GAP_PROF_SEPARADO = false;
...
const bool MARRETA_PRIOR_EDF = true;
...
MIPUnico::MIPUnico( ProblemData * aProblemData, ProblemSolution * const aProbSolInicial,
	bool *endCARREGA_SOLUCAO, bool equiv, int fase )
	: Solver( aProblemData ),
	probSolInicial_(aProbSolInicial), CARREGA_SOLUCAO(endCARREGA_SOLUCAO), xSol_(nullptr),
	optLogFileName("mipUnico"), optimized_(false)	
{
   MIPUnico::idCounter++;
...
void MIPUnico::printCabecalho(int campusId, int prioridade, int r)
{
	...
	std::cout<<"\nMIPUnico::priorProfImportante_ = " << MIPUnicoParametros::priorProfImportante_;
		...
		std::set<VariableMIPUnico*> &solMipUnico)
{
	...
	printCabecalho(campusId, prioridade, r);
	
	corrigeNroTurmas(prioridade, campusId);
	
	preencheMapDiscAlunosDemanda(campusId, prioridade, r);

	carregaSolucaoInicialFixada();

	solveStrategy(campusId, prioridade, r);
	...
	imprimeGoals();
	imprimeGrades(campusId, prioridade);
			
	std::cout<<"\nFim do tatico integrado!\n"; fflush(NULL);
}

void MIPUnico::solveStrategy(int campusId, int prioridade, int r)
{
	if (ABORDAGEM_PV_NO_FINAL)
	{
		if (ETAPA_UNICA) 
			solveMIPUnico_unico(campusId, prioridade, r);
		else if (MIPUnicoParametros::priorProfImportante_ == MIPUnicoParametros::PriorTypeOff ||
				 MIPUnicoParametros::priorProfImportante_ == MIPUnicoParametros::PriorTypeOffWeight ||
				 MIPUnicoParametros::priorProfImportante_ == MIPUnicoParametros::PriorType1 ||
				 MIPUnicoParametros::priorProfImportante_ == MIPUnicoParametros::PriorType2)
			solveMIPUnico_pduplo(campusId, prioridade, r);
		else if (MIPUnicoParametros::priorProfImportante_ == MIPUnicoParametros::PriorType3)
			solveMIPUnico_ptriplo(campusId, prioridade, r);
	}
	else
...
void MIPUnico::imprimeGoals()
{
	solMIPUnico_->imprimeGoals();
}
...
std::string MIPUnico::getEtapaName(int campusId, int prioridade, int r)
{
   ...
   if (MIPUnicoParametros::priorProfImportante_ != MIPUnicoParametros::PriorTypeOff &&
	   MIPUnicoParametros::priorProfImportante_ != MIPUnicoParametros::PriorTypeOffWeight)
   {
	   if (MIPUnicoParametros::priorProfLevel_==1)
		 solName += "_ProfLevel1";
	   if (MIPUnicoParametros::priorProfLevel_==2)
		 solName += "_ProfLevel2";
   }
...
void MIPUnico::getPrefixFileName(int type, std::string & prefix)
{
	prefix = "";
	switch (type)
	{
...
void MIPUnico::getSolEtapaFileName(int campusId, int prioridade, int r, int type, std::string & fileName, bool byCode)
{
	string prefix;
	getPrefixFileName(type, prefix);

	stringstream solName;
	solName << prefix;
	if (byCode) solName << "Code";
	solName << getSolucaoTaticoFileName(campusId, prioridade, r).c_str();
	fileName = solName.str();
}

bool MIPUnico::getSolFilePt(int campusId, int prioridade, int r, int type, ifstream & fin, bool byCode)
{
	std::string fileName;
	getSolEtapaFileName(campusId, prioridade, r, type, fileName, byCode);
...
{
	writeSolTxtById(campusId, prioridade, r, type, xSol, fase);
	//writeSolTxtByCode(campusId, prioridade, r, type, xSol, fase);
}

void MIPUnico::writeSolTxtById( int campusId, int prioridade, int r, int type, double *xSol, int fase )
{
	std::string fileNameById;
	getSolEtapaFileName(campusId, prioridade, r, type, fileNameById);	
	ofstream foutById( fileNameById, ios_base::out );
	if (!foutById)
	{
		std::cout << "\nErro em MIPUnico::writeSolTxtById( int campusId, int prioridade, int r, int type ):"
				<< "\nArquivo " << fileNameById << " nao pode ser aberto.\n";
	}
	...
	for (auto vit = vHashTatico.begin(); vit != vHashTatico.end(); vit++)
	{
		...
		if(foutById) foutById << v.toString() << " = " << value << endl;
	}

	if(foutById) foutById.close();
}

void MIPUnico::writeSolTxtByCode( int campusId, int prioridade, int r, int type, double *xSol, int fase )
{	
	std::string fileNameByCode;
	getSolEtapaFileName(campusId, prioridade, r, type, fileNameByCode, true);
	ofstream foutByCode( fileNameByCode, ios_base::out );
	if (!foutByCode)
	{
		std::cout << "\nErro em MIPUnico::writeSolTxtByCode( int campusId, int prioridade, int r, int type ):"
				<< "\nArquivo " << fileNameByCode << " nao pode ser aberto.\n";
	}

	if(!foutByCode) return;
	...
	for (auto vit = vHashTatico.begin(); vit != vHashTatico.end(); vit++)
	{
		...
		if(foutByCode) foutByCode << v.toCodeString() << " = " << value << endl;
	}

	if(foutByCode) foutByCode.close();
}
...
int MIPUnico::readSolTxt( int campusId, int prioridade, int r, int type, double *xSol, int fase )
{
	...
	ifstream finById;
	getSolFilePt(campusId, prioridade, r, type, finById);
	if (finById) fin = &finById;

	ifstream finByCode;
	if (!finById)
	{
		getSolFilePt(campusId, prioridade, r, type, finByCode, true);
		if (!finByCode) fin = &finByCode;
		byCode = true;
	}		
	
	if (!finById && !finByCode) return 0;
    ...
    for (auto vit = vHashTatico.begin(); vit != vHashTatico.end(); vit++)
    {
		...
		if (byCode) varName = v.toCodeString();
		else varName = v.toString();

        if ( varVals.find(varName) != varVals.end() )
        {
			xSol[col] = varVals[varName];										// todo: retornar o vetor idx de colunas consideradas
        }         
    }
...
void MIPUnico::addVarSol(VariableMIPUnico* const v)
{
	solMIPUnico_->addVarSol(v);
}

void MIPUnico::carregaSolucaoInicialFixada()
{
	if (!probSolInicial_) return;
	...
	probSolInicial_->getMapsDaSolucao(solTurmaProfAlunos, solCpDiscTurmaDiaDti);
	
	for (auto itSala = solTurmaProfAlunos.cbegin(); 
		itSala != solTurmaProfAlunos.cend(); itSala++)
	{
		int const cpId = itSala->first->getIdCampus();
		Campus *const cp = problemData->refCampus[cpId];

		auto finderCp = solCpDiscTurmaDiaDti.find(cp);
		if (finderCp == solCpDiscTurmaDiaDti.end()) continue; // error!

		for (auto itDisc = itSala->second.cbegin(); 
			itDisc != itSala->second.cend(); itDisc++)
		{
			Disciplina * const d = itDisc->first;

			auto finderDisc = finderCp->second.find(d);
			if (finderDisc == finderCp->second.end()) continue; // error!

			for (auto itTurma = itDisc->second.cbegin(); 
				itTurma != itDisc->second.cend(); itTurma++)
			{
				int const turma = itTurma->first;
				Professor *const p = itTurma->second.first;
				
				// Add turma + prof
				addSolAlocProfTurma(p, cp, d, turma);

				auto finderTurma = finderDisc->second.find(turma);
				if (finderTurma == finderDisc->second.end()) continue; // error!

				for (auto itDia = finderTurma->second.cbegin(); 
					itDia != finderTurma->second.cend(); itDia++)
				{
					for (auto itDti = itDia->second.cbegin(); 
						itDti != itDia->second.cend(); itDti++)
					{				
						for (auto itAluno = itTurma->second.second.cbegin(); 
							itAluno != itTurma->second.second.cend(); itAluno++)
						{
							// Add aluno + aula
							addSolAlocAlunoTurma(*itAluno, d, turma, itDia->first, *itDti);
						}				
					}
				}
			}
    ...
    for (; vit != vHashTatico.end(); vit++)
    {
         ...
         switch( v->getType() )
         {
			 ...
			 case VariableMIPUnico::V_ALUNO_CREDITOS:
				 ...
				 {
					 addSolAlocAlunoTurma(vAluno, vDisc, vTurma, vDia, ha->getInicio() );
			 ...
			 case VariableMIPUnico::V_PROF_TURMA:
				 ...
				 vTurma = v->getTurma();
				 vCampus = v->getCampus();
				 addSolAlocProfTurma(vProf, vCampus, vDisc, vTurma);
				 break;
         }
    }
...
{
   solMIPUnico_->deleteVariablesSol();
}
...
{
	// Professores virtuais e reais juntos desde o inicio, em um mesmo modelo

	bool status=1;

	MIPUnicoParametros::permiteCriarPV = true;
	status &= solveMIPUnico_v2(campusId, prioridade, r);

	return status;
}

int MIPUnico::solveMIPUnico_pduplo(int campusId, int prioridade, int r)
{
	// Modelo inicial somente com professores reais, e no final um modelo 
	// com a solução real fixada e os virtuais para as turmas necessárias

	bool status=1;

	MIPUnicoParametros::permiteCriarPV = false;
	status = solveMIPUnico_v2(campusId, prioridade, r);
...
int MIPUnico::solveMIPUnico_ptriplo(int campusId, int prioridade, int r)
{
	// Primeiro modelo somente com professores reais importantes,
	// segundo modelo com solução de professores reais importantes fixada e os profs não importantes a decidir,
	// terceiro modelo com toda solução real fixada e os virtuais para as turmas necessárias

	bool status=1;

	MIPUnicoParametros::permiteCriarPV = false;

	MIPUnicoParametros::priorProfLevel_ = 1;
	status = solveMIPUnico_v2(campusId, prioridade, r);
	
	MIPUnicoParametros::priorProfLevel_ = 2;
	status &= solveMIPUnico_v2(campusId, prioridade, r);
...
{					 			
	...
	if (ETAPA_UNICA)
	{
		status=solveMIPUnicoEtapaReal_Off(campusId, prioridade, r, CARREGA_SOL_PARCIAL);
	...
	if (!MIPUnicoParametros::permiteCriarPV)
	{
		if (MIPUnicoParametros::priorProfImportante_ == MIPUnicoParametros::PriorTypeOff ||
			MIPUnicoParametros::priorProfImportante_ == MIPUnicoParametros::PriorTypeOffWeight)
			status=solveMIPUnicoEtapaReal_Off(campusId, prioridade, r, CARREGA_SOL_PARCIAL);
		else if (MIPUnicoParametros::priorProfImportante_ == MIPUnicoParametros::PriorType1) 
			status=solveMIPUnicoEtapaReal_1(campusId, prioridade, r, CARREGA_SOL_PARCIAL);
		else if (MIPUnicoParametros::priorProfImportante_ == MIPUnicoParametros::PriorType2) 
...
{	
	if (!ETAPA_UNICA &&
		MIPUnicoParametros::priorProfImportante_ != MIPUnicoParametros::PriorTypeOff &&
		MIPUnicoParametros::priorProfImportante_ != MIPUnicoParametros::PriorTypeOffWeight) return 0;
...
int MIPUnico::solveMIPUnicoEtapaReal_1(int campusId, int prioridade, int r, bool &CARREGA_SOL_PARCIAL)
{	
	if (MIPUnicoParametros::priorProfImportante_ != MIPUnicoParametros::PriorType1) return 0;
...
int MIPUnico::solveMIPUnicoEtapaReal_2(int campusId, int prioridade, int r, bool &CARREGA_SOL_PARCIAL)
{	
	if (MIPUnicoParametros::priorProfImportante_ != MIPUnicoParametros::PriorType2) return 0;

	MIPUnicoParametros::priorProfLevel_ = MIPUnicoParametros::allPriorProfLevels_;			// considera todos os professores
	...
	MIPUnicoParametros::priorProfLevel_ = 1;			// considera somente os professores de prioridade 1
	fixaVariaveisProfNaoImportanteZero( xSol_ );
	solveMIPUnicoEtapaReal_ProfPrior( campusId, prioridade, r, CARREGA_SOL_PARCIAL );
	liberaVariaveisProfNaoImportanteZero( xSol_ );
	fixaVariaveisProfImportanteAtend( xSol_ );
	
	MIPUnicoParametros::priorProfLevel_ = 2;			// considera somente os professores de prioridade 2
	solveMIPUnicoEtapaReal_ProfPrior( campusId, prioridade, r, CARREGA_SOL_PARCIAL );
...
{	
	if (MIPUnicoParametros::priorProfImportante_ != MIPUnicoParametros::PriorType3) return 0;
...
int MIPUnico::solveMIPUnicoEtapaReal_ProfPrior(int campusId, int prioridade, int r, bool &CARREGA_SOL_PARCIAL)
{
	int stat=1;
	stat &= solveMaxAtend( campusId, prioridade, r, CARREGA_SOL_PARCIAL, xSol_ );				
	stat &= solveMinTurmas( campusId, prioridade, r, CARREGA_SOL_PARCIAL, xSol_ );
	stat &= solveMinDeslocProf( campusId, prioridade, r, CARREGA_SOL_PARCIAL, xSol_ );		
	stat &= solveMinFasesDoDiaProf( campusId, prioridade, r, CARREGA_SOL_PARCIAL, xSol_ );
...
void MIPUnico::fixaVariaveisProfNaoImportanteZero(double * const xS)
{
	if (MIPUnicoParametros::priorProfImportante_ != MIPUnicoParametros::PriorType2) return;
	...
	for ( ; vit != vHashTatico.end(); vit++ )
	{
		...
		if (v.getType() == VariableMIPUnico::V_PROF_TURMA)
		if (!v.getProfessor()->eVirtual() && value == 0)
		if (!priorProf(v.getProfessor()))
		{
			valsOrig[nBds] = value;
			idxs[nBds] = vit->second;
			vals[nBds] = value;
...
void MIPUnico::liberaVariaveisProfNaoImportanteZero(double * const xS)
{
	// todo: soltar exatemente o que foi inicialmente liberado
...
void MIPUnico::fixaVariaveisProfImportanteAtend(double * const xS)
{
	if (!MIPUnicoParametros::fixarSolucaoProfPrior1_) return;
...
int MIPUnico::getTimeLimit(int fase)
{
	switch (fase)
	{
		case (MIPUnicoParametros::MIP_GARANTE_SOL):
			return MIPUnicoParametros::timeLimitGeneral;
		case (MIPUnicoParametros::MIP_MAX_ATEND):
			return MIPUnicoParametros::timeLimitMaxAtend;
		case (MIPUnicoParametros::MIP_MIN_VIRT):
			return MIPUnicoParametros::timeLimitMinProfVirt;
		case (MIPUnicoParametros::MIP_MIN_TURMAS_COMPART):
			return MIPUnicoParametros::timeLimitMinTurmas;
		case (MIPUnicoParametros::MIP_MIN_DESLOC_PROF):
			return MIPUnicoParametros::timeLimitMinDeslocProf_;
		case (MIPUnicoParametros::MIP_MIN_FASE_DIA_PROF):
			return MIPUnicoParametros::timeLimitMinFaseDiaProf_;
		case (MIPUnicoParametros::MIP_MIN_GAP_PROF):
			return MIPUnicoParametros::timeLimitMinGapProf;
		case (MIPUnicoParametros::MIP_MARRETA):
			return MIPUnicoParametros::timeLimitMaxAtend;
		default:
			return MIPUnicoParametros::timeLimitGeneral;
...
int MIPUnico::getTimeLimitNoImprov(int fase)
{
	switch (fase)
	{
		case (MIPUnicoParametros::MIP_GARANTE_SOL):
			return MIPUnicoParametros::timeLimitGeneralSemMelhora;
		case (MIPUnicoParametros::MIP_MAX_ATEND):
			return MIPUnicoParametros::timeLimitMaxAtendSemMelhora;
		case (MIPUnicoParametros::MIP_MIN_VIRT):
			return MIPUnicoParametros::timeLimitMinProfVirtSemMelhora;
		case (MIPUnicoParametros::MIP_MIN_TURMAS_COMPART):
			return MIPUnicoParametros::timeLimitMinTurmasSemMelhora;
		case (MIPUnicoParametros::MIP_MIN_DESLOC_PROF):
			return MIPUnicoParametros::timeLimitMinDeslocProfSemMelhora_;
		case (MIPUnicoParametros::MIP_MIN_FASE_DIA_PROF):
			return MIPUnicoParametros::timeLimitMinFaseDiaProfSemMelhora_;
		case (MIPUnicoParametros::MIP_MIN_GAP_PROF):
			return MIPUnicoParametros::timeLimitMinGapProfSemMelhora;
		case (MIPUnicoParametros::MIP_MARRETA):
			return MIPUnicoParametros::timeLimitMaxAtendSemMelhora;
		default:
			return MIPUnicoParametros::timeLimitGeneralSemMelhora;
...
int MIPUnico::polishAndOptimize( int campusId, int prioridade, int r, bool& CARREGA_SOL_PARCIAL, double *xS, int fase )
{
	if ( CARREGA_SOL_PARCIAL )
	{
		...
		int statusReadBin = readSolTxt(campusId, prioridade, r, fase, xS, 0 );
		if ( !statusReadBin )
		{
			CARREGA_SOL_PARCIAL=false;
		}
		else writeSolTxt( campusId, prioridade, r, fase, xS, 0 );
	}
	if ( !CARREGA_SOL_PARCIAL )
	{
		// GENERATES SOLUTION
		...
		if (!polishing)
		{
			...
			cb_data.timeLimit = timeLimitNoImprov;
			cb_data.gapMax = 40;
		...
		Indicadores::printEscolaIndicadores(
			...
			goal->isOpt(), goal->getPercFixedFinal(), Utilidade::getTimeStr(goal->getRunTime()), goal->getStopCriteriaStr());
...
void MIPUnico::updateAllGoals(GoalStatus* const goal)
{   
	double objValueAtend=0;
	double objValueNrTurmas=0;
	double objValueDesloc=0;
	double objValueFasesDias=0;
	double objValueGap=0;
    ...
    for (; vit != vHashTatico.end(); vit++)
    {
		 ...
		 objValueAtend += getCoefObjMaxAtend(v) * value;
		 objValueNrTurmas += getCoefObjMinTurmas(v) * value;
		 objValueDesloc += getCoefObjMinDeslocProf(v) * value;
		 objValueFasesDias += getCoefObjMinFasesDoDiaProf(v) * value;
		 objValueGap += getCoefObjMinGapProf(v) * value;
	}

	goal->setValueAtend(objValueAtend);
	goal->setValueNrTurmas(objValueNrTurmas);
	goal->setValueDesloc(objValueDesloc);
	goal->setValueFasesDias(objValueFasesDias);
	goal->setValueGap(objValueGap);
}
...
void MIPUnico::copyFinalSolution(std::set<VariableMIPUnico*> &solMipUnico)
{
	solMIPUnico_->copyFinalSolution(solMipUnico);
}
...
int MIPUnico::solveMaxAtendMarreta( int campusId, int prioridade, int r, bool& CARREGA_SOL_PARCIAL, double *xS )
{
	...
	polishAndOptimize(campusId, prioridade, r, CARREGA_SOL_PARCIAL, xS, MIPUnicoParametros::MIP_MARRETA);		

	// ------------------------------------------------------------------------------------
	// FIXA SOLUÇÃO OBTIDA
	//fixaSolMaxAtendMarreta(xS);
	fixaSolMaxAtendMarretaConstr(xS);
...
int MIPUnico::optimizeMaxAtendMarreta( int campusId, int prioridade, int r, bool& CARREGA_SOL_PARCIAL, double *xS )
{
	...
	if ( !CARREGA_SOL_PARCIAL )
	{
		...
		{
			lp->setNumIntSols(100000000);
			lp->setTimeLimit(MIPUnicoParametros::timeLimitMaxAtend);
			lp->setPreSolveIntensity(OPT_LEVEL2);
			lp->setMIPStartAlg(METHOD_BARRIERANDCROSSOVER);
			lp->setMIPEmphasis(1);
			lp->setSymetry(-1);
			...
			#if defined SOLVER_GUROBI && defined USAR_CALLBACK
			cb_data.timeLimit = MIPUnicoParametros::timeLimitMaxAtendSemMelhora;
			cb_data.gapMax = 40;
			lp->setCallbackFunc( &timeWithoutChangeCallback, &cb_data );
			#endif
	...
	for (auto vit = vHashTatico.begin(); vit != vHashTatico.end(); vit++)
	{
		...
		else if (v.getType() == VariableMIPUnico::V_SLACK_DEMANDA_ALUNO)
		{
			if (value == 0)					// Atendido
			{
				...
				else if (v.getAluno()->ehFormando())				// Aluno de medicina
				{
					AlunoDemanda* const ad = v.getAluno()->getAlunoDemanda(v.getDisciplina()->getId());
					if (ad->getExigeEquivalenciaForcada())	// Caso de 5 tempos da medicina
					{
...
bool MIPUnico::fixaSolMaxAtendMarretaConstr(double* const xS)
{	
	set< pair<int,double> > vars;
	...
	for (auto vit = vHashTatico.begin(); vit != vHashTatico.end(); vit++)
	{
		...
		rhs += numCred * value;	// nro creds NÃO atendidos
		nnz++;
	...
	stringstream name;
	name << "C_FIX_SOL_ATEND_MARRETA_";
	OPT_ROW row( nnz, OPT_ROW::LESS, rhs, (char*) name.str().c_str() );

	for (auto itVar = vars.cbegin(); itVar != vars.cend(); itVar++)
	{		
		row.insert((itVar)->first, (itVar)->second);		
	}
	if (row.getnnz() > 0)
	{
		chged &= lp->addRow(row);
...
int MIPUnico::solveMaxAtend( int campusId, int prioridade, int r, bool& CARREGA_SOL_PARCIAL, double *xS )
{
	...
	polishAndOptimize(campusId, prioridade, r, CARREGA_SOL_PARCIAL, xS, MIPUnicoParametros::MIP_MAX_ATEND);
...
double MIPUnico::getCoefObjMaxAtend(VariableMIPUnico v)
{
	...
	if (considerarDivisaoPriorProf() && !priorProfLivre()) 
	{
		int nr = v.getDisciplina()->getNroProfRealImportHabilit(MIPUnicoParametros::priorProfLevel_);
		coef = (nr>0 ? v.getDisciplina()->getTotalCreditos() : 0);
	}

	if (MIPUnicoParametros::priorProfImportante_ == MIPUnicoParametros::PriorTypeOffWeight)
	{
		int nrImport = v.getDisciplina()->getNroProfRealImportHabilit(1);
		int nrNaoImport = v.getDisciplina()->getNroProfRealImportHabilit(2);
		coef = (nrImport>nrNaoImport ? 2*coef : coef);
	}
...
bool MIPUnico::chgObjMaxAtend()
{
	...
	if (considerarAdicional && MIPUnicoParametros::permiteCriarPV)
	{
...
int MIPUnico::optimizeMaxAtend( int campusId, int prioridade, int r, bool& CARREGA_SOL_PARCIAL, double *xS )
{	
	static int fase=0;
	fase++;
...
bool MIPUnico::fixaSolVariaveisAtend(double* const xS)
{
	int *idxs = new int[lp->getNumCols()*2];
	double *vals = new double[lp->getNumCols()*2];
	BOUNDTYPE *bds = new BOUNDTYPE[lp->getNumCols()*2];
    ...
    bool chg = lp->chgBds(nBds,idxs,bds,vals);
...
bool MIPUnico::fixaSolMaxAtend(double* const xS)
{
	...
	stringstream constr;
	constr << "C_FIX_SOL_MIN_ATEND_PRIORPROF" << MIPUnicoParametros::priorProfLevel_;
...
void MIPUnico::printNaoAtendimentos(double* const xS)
{
	...
	set<VariableMIPUnico> varsAtualImport;
	set<VariableMIPUnico> varsOutros;
	...
	for (auto vit = vHashTatico.begin(); vit != vHashTatico.end(); vit++)
	{
		...
		int nrImport = v.getDisciplina()->getNroProfRealImportHabilit(MIPUnicoParametros::priorProfLevel_);
		if (nrImport)
		{
			nrCredsImport += v.getDisciplina()->getTotalCreditos();
			varsAtualImport.insert(v);
		}
		else
		{
			nrCredsOutros += v.getDisciplina()->getTotalCreditos();
			varsOutros.insert(v);
		}
	}
	...
	out << "Total de creditos nao atendidos: " << nrCredsImport + nrCredsOutros;
	out << "\nDisciplina \t - \t Aluno\n";
	
	out << "\nImportancia: " << MIPUnicoParametros::priorProfLevel_ 
		<< "\t qtd creds: " << nrCredsImport 
		<< " (" << varsAtualImport.size() << " alDem)";
	for (auto vit = varsAtualImport.begin(); vit != varsAtualImport.end(); vit++)
	{
		out << "\n" << vit->getDisciplina()->getCodigo() 
			<< " (" << vit->getDisciplina()->getId() << ")  -  " 
			<< vit->getAluno()->getNomeAluno() << " (" << vit->getAluno()->getAlunoId() << ")";
	}
	out << "\n\nDemais importancias: " 
		<< "\t qtd creds: " << nrCredsOutros 
		<< " (" << varsOutros.size() << " alDem)";
	for (auto vit = varsOutros.begin(); vit != varsOutros.end(); vit++)
	{
		out << "\n" << vit->getDisciplina()->getCodigo() 
			<< " (" << vit->getDisciplina()->getId() << ")  -  " 
			<< vit->getAluno()->getNomeAluno() << " (" << vit->getAluno()->getAlunoId() << ")";
...
int MIPUnico::solveMinProfVirt( int campusId, int prioridade, int r, bool& CARREGA_SOL_PARCIAL, double *xS )
{
	...
	polishAndOptimize(campusId, prioridade, r, CARREGA_SOL_PARCIAL, xS, MIPUnicoParametros::MIP_MIN_VIRT);
...
int MIPUnico::optimizeMinProfVirt( int campusId, int prioridade, int r, bool& CARREGA_SOL_PARCIAL, double *xS )
{
	...
	if ( !CARREGA_SOL_PARCIAL )
	{
		...
		{
			lp->setNumIntSols(100000000);
			lp->setTimeLimit(MIPUnicoParametros::timeLimitMinProfVirt);
			lp->setPreSolveIntensity(OPT_LEVEL2);
			lp->setMIPStartAlg(METHOD_BARRIERANDCROSSOVER);
			lp->setMIPEmphasis(1);
			lp->setSymetry(-1);
			...
			#if defined SOLVER_GUROBI && defined USAR_CALLBACK
			cb_data.timeLimit = MIPUnicoParametros::timeLimitMinProfVirtSemMelhora;
			cb_data.gapMax = 40;
			lp->setCallbackFunc( &timeWithoutChangeCallback, &cb_data );
			#endif
...
int MIPUnico::solveMinTurmas( int campusId, int prioridade, int r, bool& CARREGA_SOL_PARCIAL, double *xS )
{
	...
	polishAndOptimize(campusId, prioridade, r, CARREGA_SOL_PARCIAL, xS, MIPUnicoParametros::MIP_MIN_TURMAS_COMPART);
...
double MIPUnico::getCoefObjMinTurmas(VariableMIPUnico v)
{
...
bool MIPUnico::chgObjMinTurmas()
{
	...
	for ( ; vit != vHashTatico.end(); vit++ )
	{
		...
		double coef = getCoefObjMinTurmas(v);
...
int MIPUnico::optimizeMinTurmas( int campusId, int prioridade, int r, bool& CARREGA_SOL_PARCIAL, double *xS )
{
	...
	if ( !CARREGA_SOL_PARCIAL )
	{
		...
		{
			lp->setNumIntSols(100000000);
			lp->setTimeLimit(MIPUnicoParametros::timeLimitMinTurmas);
			lp->setPreSolveIntensity(OPT_LEVEL2);
			lp->setMIPStartAlg(METHOD_BARRIERANDCROSSOVER);
			lp->setMIPEmphasis(1);
			lp->setSymetry(-1);
			...
			#if defined SOLVER_GUROBI && defined USAR_CALLBACK
			cb_data.timeLimit = MIPUnicoParametros::timeLimitMinTurmasSemMelhora;
			cb_data.gapMax = 40;
			lp->setCallbackFunc( &timeWithoutChangeCallback, &cb_data );
			#endif
...
{
	if (MIPUnicoParametros::priorProfImportante_ != MIPUnicoParametros::PriorType2) return false;
...
bool MIPUnico::fixaSolMinTurmasAbordPvFinal(double* const xS)
{
	...
	{
		stringstream constr;
		constr << "C_FIX_SOL_MAX_TURMAS_COMPARTILH_PROF_REAL_";
...
int MIPUnico::solveMinDeslocProf( int campusId, int prioridade, int r, bool& CARREGA_SOL_PARCIAL, double *xS )
{
	...
	polishAndOptimize(campusId, prioridade, r, CARREGA_SOL_PARCIAL, xS, MIPUnicoParametros::MIP_MIN_DESLOC_PROF);
...
double MIPUnico::getCoefObjMinDeslocProf(VariableMIPUnico v)
{
	double coef = 0.0;
	if (v.getType() != VariableMIPUnico::V_PROF_DESLOC &&
		v.getType() != VariableMIPUnico::V_PROF_UNID)
		 return coef;
	if (!priorProf(v.getProfessor()))
		return coef;
	...
	{     
		...
		if (tempoMin >= MIPUnicoParametros::maxTempoDeslocCurto_) 
			coef = tempoMin;
	}
	...
	if (MIPUnicoParametros::priorProfImportante_ == MIPUnicoParametros::PriorTypeOffWeight &&
		v.getProfessor()->getImportancia()==1)
	{
		coef *= 2;
	}
...
bool MIPUnico::chgObjMinDeslocProf()
{
	...
	for ( ; vit != vHashTatico.end(); vit++ )
	{
		...
		double coef = getCoefObjMinDeslocProf(v);
...
int MIPUnico::optimizeMinDeslocProf(int campusId, int prioridade, int r, bool& CARREGA_SOL_PARCIAL, double *xS)
{
	...
	{		
		...
		{
			lp->setNumIntSols(100000000);
			lp->setTimeLimit(MIPUnicoParametros::timeLimitMinDeslocProf_);
			lp->setPreSolveIntensity(OPT_LEVEL2);
			lp->setMIPStartAlg(METHOD_BARRIERANDCROSSOVER);
			lp->setMIPEmphasis(1);
			lp->setSymetry(-1);
			...
			#if defined SOLVER_GUROBI && defined USAR_CALLBACK
			cb_data.timeLimit = MIPUnicoParametros::timeLimitMinDeslocProfSemMelhora_;
			cb_data.gapMax = 40;
			lp->setCallbackFunc( &timeWithoutChangeCallback, &cb_data );
			#endif
...
bool MIPUnico::fixaSolMinDeslocProf(double* const xS)
{
	...
	mapSolMinDeslocProf(xS, solProfDiaDeslocUsado, solProfDiaUnidUsada);

	chged &= fixaSolMinDiaDeslocProf(solProfDiaDeslocUsado);
	chged &= fixaSolMinSumDeslocProf(solProfDiaDeslocUsado);
	chged &= fixaSolMinUnidDiaProf(solProfDiaUnidUsada);
...
void MIPUnico::mapSolMinDeslocProf(double* const xS,
		map<Professor*, map<int, map<VariableMIPUnico, pair<int,double>>>> &solProfDiaDeslocUsado,
		map<Professor*, std::set< pair<int,double> >> &solProfDiaUnidUsada)
{	
	for (auto vit = vHashTatico.begin(); vit != vHashTatico.end(); vit++)
...
bool MIPUnico::fixaSolMinDiaDeslocProf(
...
bool MIPUnico::fixaSolMinSumDeslocProf(
		map<Professor*, map<int, map<VariableMIPUnico, pair<int,double>>>> const &solProfDiaDeslocUsado)
{
	bool chged = true;
...
bool MIPUnico::fixaSolMinUnidDiaProf(
...
{
	if (!MIPUnicoParametros::minimizarProfFaseDoDiaUsada_ && !MIPUnicoParametros::minimizarProfDiaUsado_)
		return 1;
	...
	polishAndOptimize(campusId, prioridade, r, CARREGA_SOL_PARCIAL, xS, MIPUnicoParametros::MIP_MIN_FASE_DIA_PROF);
...
double MIPUnico::getCoefObjMinFasesDoDiaProf(VariableMIPUnico v)
{
	double coef = 0.0;
	if (v.getType() != VariableMIPUnico::V_PROF_FASE_DIA_USADA &&
		v.getType() != VariableMIPUnico::V_PROF_DIA_USADO)
		return coef;
	if (v.getProfessor()->eVirtual())
		return coef;

	if (v.getType() == VariableMIPUnico::V_PROF_FASE_DIA_USADA && MIPUnicoParametros::minimizarProfFaseDoDiaUsada_)
	if (!v.getProfessor()->eVirtual() && priorProf(v.getProfessor()))
	{     
		coef = 1.0;
	}

	if (v.getType() == VariableMIPUnico::V_PROF_DIA_USADO && MIPUnicoParametros::minimizarProfDiaUsado_)
	if (!v.getProfessor()->eVirtual() && priorProf(v.getProfessor()))
	{     
		coef = 1.0;
	}
...
bool MIPUnico::chgObjMinFasesDoDiaProf()
{
	...
	for ( ; vit != vHashTatico.end(); vit++ )
	{
		...
		double coef = getCoefObjMinFasesDoDiaProf(v);
...
int MIPUnico::optimizeMinFasesDoDiaProf( int campusId, int prioridade, int r, bool& CARREGA_SOL_PARCIAL, double *xS )
{
	...
	{		
		...
		{
			lp->setNumIntSols(100000000);
			lp->setTimeLimit(MIPUnicoParametros::timeLimitMinFaseDiaProf_);
			lp->setPreSolveIntensity(OPT_LEVEL2);
			lp->setMIPStartAlg(METHOD_BARRIERANDCROSSOVER);
			lp->setMIPEmphasis(1);
			lp->setSymetry(-1);
			...
			#if defined SOLVER_GUROBI && defined USAR_CALLBACK
			cb_data.timeLimit = MIPUnicoParametros::timeLimitMinFaseDiaProfSemMelhora_;
			cb_data.gapMax = 40;
			lp->setCallbackFunc( &timeWithoutChangeCallback, &cb_data );
			#endif
...
{
	if (!MIPUnicoParametros::minimizarProfFaseDoDiaUsada_)
		return true;
	...
	for ( ; itProf != varsProfDiaFaseUsada.end(); itProf++ )
	{
		...
		if (!priorProf(itProf->first)) continue;
...
{
	if (!MIPUnicoParametros::minimizarProfDiaUsado_)
		return true;
...
int MIPUnico::solveMinGapProf( int campusId, int prioridade, int r, bool& CARREGA_SOL_PARCIAL, double *xS )
{
	...
	polishAndOptimize(campusId, prioridade, r, CARREGA_SOL_PARCIAL, xS, MIPUnicoParametros::MIP_MIN_GAP_PROF);
...
double MIPUnico::getCoefObjMinGapProf(VariableMIPUnico v)
{
	double coef = 0.0;
	if (v.getType() != VariableMIPUnico::V_FOLGA_GAP_PROF &&
		v.getType() != VariableMIPUnico::V_PROF_DESLOC)
		return coef;
	if (v.getProfessor()->eVirtual())
		return coef;
...
bool MIPUnico::chgObjMinGapProf()
{
	...
	for ( ; vit != vHashTatico.end(); vit++ )
	{
		...
		double coef = getCoefObjMinGapProf(v);
...
int MIPUnico::optimizeMinGapProf( int campusId, int prioridade, int r, bool& CARREGA_SOL_PARCIAL, double *xS )
{
	...
	{		
		...
		{
			lp->setNumIntSols(100000000);
			lp->setTimeLimit(MIPUnicoParametros::timeLimitMinGapProf);
			lp->setPreSolveIntensity(OPT_LEVEL2);
			lp->setMIPStartAlg(METHOD_BARRIERANDCROSSOVER);
			lp->setMIPEmphasis(1);
			lp->setSymetry(-1);
			...
			#if defined SOLVER_GUROBI && defined USAR_CALLBACK
			cb_data.timeLimit = MIPUnicoParametros::timeLimitMinGapProfSemMelhora;
			cb_data.gapMax = 40;
			lp->setCallbackFunc( &timeWithoutChangeCallback, &cb_data );
			#endif
...
{		
	...
	if ( !CARREGA_SOL_PARCIAL )
	{
		...
		{
			lp->setNumIntSols(0);
			lp->setTimeLimit(MIPUnicoParametros::timeLimitGeneral);
			lp->setPreSolveIntensity(OPT_LEVEL2);
			lp->setMIPStartAlg(METHOD_BARRIERANDCROSSOVER);
			lp->setHeurFrequency(0.8);
			lp->setMIPEmphasis(1);
			...
			lp->setCuts(2);
			#if defined SOLVER_GUROBI && defined USAR_CALLBACK
			cb_data.timeLimit = MIPUnicoParametros::timeLimitGeneralSemMelhora;
			cb_data.gapMax = 30;
			lp->setCallbackFunc( &timeWithoutChangeCallback, &cb_data );
			#endif
			lp->updateLP();
...
{
	if (!considerarDivisaoPriorProf()) return true;

	return (MIPUnicoParametros::priorProfLevel_ == MIPUnicoParametros::allPriorProfLevels_);
}

bool MIPUnico::considerarDivisaoPriorProf()
{
	if (MIPUnicoParametros::priorProfImportante_ == MIPUnicoParametros::PriorTypeOff ||
		MIPUnicoParametros::priorProfImportante_ == MIPUnicoParametros::PriorTypeOffWeight)
		return false;
...
bool MIPUnico::priorProf(Professor* const professor, bool ouMenor)
{
	if (!considerarDivisaoPriorProf())
		return true;
	if (MIPUnicoParametros::priorProfLevel_ == MIPUnicoParametros::allPriorProfLevels_)
		return true;
	return (professor->getImportancia() == MIPUnicoParametros::priorProfLevel_ ||
		   (professor->getImportancia() < MIPUnicoParametros::priorProfLevel_ && ouMenor));
}
...
bool MIPUnico::priorDisc(Disciplina* const disciplina, bool ouMenor)
{
	...
	if (considerarDivisaoPriorProf() && !priorProfLivre() && !ehMarreta(disciplina)) 
	{
		int nr = disciplina->getNroProfRealImportHabilit(MIPUnicoParametros::priorProfLevel_, ouMenor);
		if (nr == 0)
...
bool MIPUnico::haProfHabilitNoDiaHor(Disciplina* const disc, int dia, HorarioAula* const ha)
{
	...
	{
		if (MIPUnicoParametros::priorProfImportante_ == MIPUnicoParametros::PriorType3 &&
			!ehMarreta(disc) &&
			!priorProf(*itProf,true))
			continue;
...
{
	int numVars = 0;
	if ( MIPUnicoParametros::consideraDivCredDisc == ParametrosPlanejamento::Off )
		return numVars;
	...
	{		
	    ...
	    if ( !v.getDisciplina()->possuiRegraCredComMultiDias() )
		    continue;
...
{		
	if ( MIPUnicoParametros::consideraDivCredDisc != ParametrosPlanejamento::Weak )
	   return 0;
	...
	{		
		...
		double coef = MIPUnicoParametros::pesoDivCred;
		...
		ITERA_GGROUP_LESSPTR( itAlDemanda, aluno->demandas, AlunoDemanda )
		{
			...
			{								
				...
				if ( itAlDemanda->getPrioridade() == 1 )
					coef = MIPUnicoParametros::pesoFD * disciplina->getTotalCreditos();
				...
				for ( auto itProf = itTurma->second.cbegin(); itProf != itTurma->second.cend(); itProf++ )
				{
					...
					if ( vHashTatico.find( v ) == vHashTatico.end() )
					{
						...
						if ( professor->eVirtual() )
							coef = MIPUnicoParametros::pesoCredPV * disciplina->getTotalCreditos();
						if ( MIPUnicoParametros::minimizarCustoProf )
	...
	for ( auto itCp = vars_turma_aula.begin(); itCp != vars_turma_aula.end(); itCp++ )
	{
		for ( auto itDisc = itCp->second.begin(); itDisc != itCp->second.end(); itDisc++ )
		{
					...
					itProf != itCp->first->professores.end(); itProf++ )
			{
				...
				if (!priorProf(*itProf,true) && !ehMarreta(itDisc->first) &&
					MIPUnicoParametros::priorProfImportante_ == MIPUnicoParametros::PriorType3)
				{				
					continue;
				}
		...
		for ( auto itDisc = itCp->second.begin(); itDisc != itCp->second.end(); itDisc++ )
		{
			...
			for ( auto itTurma = itDisc->second.begin(); itTurma != itDisc->second.end(); itTurma++ )
			{
				...
				for ( auto itProf = allProfsValidos->begin(); itProf != allProfsValidos->end(); itProf++ )
				{
					...
					// Se a turma já existe com outro prof real alocado, pula.
					if (turmaExiste && profAloc && professor!=profAloc) continue;
					...
					for ( auto itDia = itTurma->second.begin(); itDia != itTurma->second.end(); itDia++ )
					{
						for ( auto itDti = itDia->second.begin(); itDti != itDia->second.end(); itDti++ )
						{
							for ( auto itVar = itDti->second.begin(); itVar != itDti->second.end(); itVar++ )
							{
								...
								if ( !ABORDAGEM_PV_NO_FINAL &&
									 MIPUnicoParametros::filtroPVHorCompl_ &&
									 professor->eVirtual() && 
	...
	for(auto itProf = vars_prof_dia_fase_dt.begin(); itProf != vars_prof_dia_fase_dt.end(); ++itProf)
	{
		...
		for(auto itDia = itProf->second.begin(); itDia != itProf->second.end(); ++itDia)
		{
			for(auto itFase = itDia->second.begin(); itFase != itDia->second.end(); ++itFase)
			{
				...
				const double coef = MIPUnicoParametros::pesoGapProf;
				const int lb = 0;
		...
		for(auto itDia = itProf->second.begin(); itDia != itProf->second.end(); ++itDia)
		{
			...
			{	
				for(auto itUnid2 = itDia->second.begin(); itUnid2 != itDia->second.end(); ++itUnid2)
				{
					...
					double coef = MIPUnicoParametros::pesoDeslocProf * tempoDesloc;
	...
	for(auto itAluno = vars_aluno_aula.begin(); itAluno != vars_aluno_aula.end(); ++itAluno)
	{
		...
		for(auto itDia = itAluno->second.begin(); itDia != itAluno->second.end(); ++itDia)
		{
			...
			for(auto itDti = itDia->second.begin(); itDti != itDia->second.end(); ++itDti)
			{
				...
				if (MIPUnicoParametros::ignorarGapAlunoContraTurno_ && somenteNoContraTurno) continue;
...
int MIPUnico::criaVariavelFolgaGapAlunoAPartirDeV(void)
{
	...
	for(auto itAluno = vars_aluno_dia_dt.begin(); itAluno != vars_aluno_dia_dt.end(); ++itAluno)
	{
		...
		for(auto itDia = itAluno->second.begin(); itDia != itAluno->second.end(); ++itDia)
		{
			...
			const double coef = MIPUnicoParametros::pesoGapAluno;
			const int lb = 0;
			const int ub = dtfMinutes - dtiMinutes;
...
int MIPUnico::criarVariavelFolgaMinCredsDiaAluno()
{
	...
	if ( MIPUnicoParametros::considerarMinCredDiaAluno != ParametrosPlanejamento::Weak )
		return numVars;
	...
	for(auto itAluno = vars_aluno_aula.begin(); itAluno != vars_aluno_aula.end(); ++itAluno)
	{
		...
		int maxFolga = aluno->getNrMedioCredsDia() - MIPUnicoParametros::desvioMinCredDiaAluno;

		const double coef = MIPUnicoParametros::pesoMinCredDiaAluno;
		const int lb = 0;
...
{
   int num_vars = 0;
   double coeff = MIPUnicoParametros::pesoCHAntProf;
...
int MIPUnico::criaRestricoesTatico( int campusId, int prioridade, int r )
{
	...
	timer.start();
	restricoes += criaRestricaoMax1DeslocProfessor();
	timer.stop();
	dif = timer.getCronoCurrSecs();
	...
	timer.start();
	restricoes += criaRestricaoProfBuracoEntreFases();
	timer.stop();
	dif = timer.getCronoCurrSecs();
	std::cout << "numRest criaRestricaoProfBuracoEntreFases: " << (restricoes - numRestAnterior) <<" "<<dif <<" sec" << std::endl;
	numRestAnterior = restricoes;
	
	timer.start();
	restricoes += criaRestricaoProfMinCredsDiaUsado();
	timer.stop();
	dif = timer.getCronoCurrSecs();
	std::cout << "numRest criaRestricaoProfMinCredsDiaUsado: " << (restricoes - numRestAnterior) <<" "<<dif <<" sec" << std::endl;
	numRestAnterior = restricoes;
...
{		
	...
	for(auto itDtiV = mapDiaVarsV.cbegin(); itDtiV != mapDiaVarsV.cend(); ++itDtiV)
	{
		...
		for(auto itCpV=itDtiV->second.cbegin(); itCpV != itDtiV->second.cend(); ++itCpV)
		{
			for(auto itDiscV=itCpV->second.cbegin(); itDiscV != itCpV->second.cend(); ++itDiscV)
			{
				if (MIPUnicoParametros::ignorarGapAlunoContraTurno_)		
		...
		if (MIPUnicoParametros::ignorarGapAlunoContraTurno_ && !achouVi && !achouVa) continue;
...
{
	int restricoes = 0;	
	if (MIPUnicoParametros::considerarMinCredDiaAluno == ParametrosPlanejamento::Off)
		return restricoes;
...
int MIPUnico::criaRestricaoMax1DeslocProfessor()
{
   int restricoes = 0;
   char name[ 200 ];
			...
			for( auto itUnidAtual = itDia->second.cbegin(); itUnidAtual != itDia->second.cend(); itUnidAtual++ )
			{
				...
				for( ; itUnidOrig != itDia->second.cend(); itUnidOrig++ )
				{
					...
					for( ; itUnidDest != itDia->second.cend(); itUnidDest++ )
					{
						...
						if (MIPUnicoParametros::limitar1DeslocSoUnidLonge_)
						{
							...
							if (tempo_minimo < MIPUnicoParametros::maxTempoDeslocCurto_) continue; // unidades proximas
						}
...
/*
	...
	sum[u1]sum[u2] desloc_{p,t,u1,u2} <= 1   tal que |u1,u2| >= MIPUnicoParametros::maxTempoDeslocCurto_
*/
...
/*
	...
	sum[t]sum[u1]sum[u2] desloc_{p,t,u1,u2} <= 2   tal que |u1,u2| >= MIPUnicoParametros::maxTempoDeslocCurto_
*/
int MIPUnico::criaRestricaoMaxDeslocLongoSemanaProfessor()
{
   ...
   if (!problemData->parametros->minimizar_desloc_prof)
	   return restricoes;
   if (!MIPUnicoParametros::limitarDeslocUnidLongeSemana_)
	   return restricoes;
   ...
   for( auto itProf = vars_prof_desloc.cbegin(); itProf != vars_prof_desloc.cend(); itProf++ )
   {
	   ...
	   {				
			...
			OPT_ROW row( 10, OPT_ROW::LESS, MIPUnicoParametros::maxDeslocUnidLongeSemana_, name );
...
{	
	// Limita nr maximo de unidades independente da distancia entra elas!!!
    ...
    if (!problemData->parametros->minimizar_desloc_prof)
	   return restricoes;
	if (!MIPUnicoParametros::limitarNrUnidsProfDia_)
	   return restricoes;
	...
	for(auto itProf = vars_prof_dia_unid.begin(); itProf != vars_prof_dia_unid.end(); ++itProf)
	{
		...
		{		
			...
			sprintf( name, "%s", c.toString(etapa).c_str() );
			OPT_ROW row( 50, OPT_ROW::LESS, MIPUnicoParametros::MaxUnidProfDia_, name );
...
int MIPUnico::criaRestricaoProfBuracoEntreFases()
{
	int restricoes = 0;
	if (!MIPUnicoParametros::minimizarGapProfEntreFases_)
		return restricoes;
	...
	for ( ; itVarsProf != varsProfDiaFaseHiHf.end(); itVarsProf++ )
	{
		...
		// Procura variaveis ptf para o professor
		auto finderProfPTF = varsProfDiaFaseUsada.find(professor);
		if (finderProfPTF == varsProfDiaFaseUsada.end()) continue;
		...
		for ( ; itVarsDia != itVarsProf->second.end(); itVarsDia++ )
		{
			...
			// Pelo menos 2 fases no dia
			if (itVarsDia->second.size() <= 1) continue;

			// Procura variaveis ptf para o dia
			auto finderDiaPTF = finderProfPTF->second.find(dia);
			if (finderDiaPTF == finderProfPTF->second.end()) continue;

			auto itVarsFase = std::next(itVarsDia->second.begin());			
			for ( ; itVarsFase != itVarsDia->second.end(); itVarsFase++ )
			{
				int fase = itVarsFase->first;

				auto itVarsFaseAnt = std::prev(itVarsFase);
				int faseAnt = itVarsFaseAnt->first;

				// Somente fases consecutivas
				if (fase-faseAnt != 1) continue;

				int nrCredsDispFaseAnt = professor->getNrCredDispFaseDia(dia, faseAnt);
				int nrCredsDispFase = professor->getNrCredDispFaseDia(dia, fase);

				// Somente fases do dia com muita disponibilidade
				if (nrCredsDispFaseAnt <= MIPUnicoParametros::minCredDispFaseMinGapProf_ ||
					nrCredsDispFase <= MIPUnicoParametros::minCredDispFaseMinGapProf_) continue;

				// Procura variaveis ptf para a fase
				auto finderFasePTF = finderDiaPTF->second.find(fase);
				if (finderFasePTF == finderDiaPTF->second.end()) continue;
				auto finderFaseAntPTF = finderDiaPTF->second.find(faseAnt);
				if (finderFaseAntPTF == finderDiaPTF->second.end()) continue;
				
				// Colunas ptf
				int colPtf = finderFasePTF->second;
				int colPtfAnt = finderFaseAntPTF->second;

				ConstraintMIPUnico c;
				c.reset();
				c.setType(ConstraintMIPUnico::C_PROF_GAP_ENTRE_FASE);
				c.setProfessor(professor);
				c.setDia(dia);
				c.setFaseDoDia(fase);
				...
				//double bigM = (dif - MIPUnicoParametros::MaxGapEntreFase_) / 2;
				//bigM = (int) (bigM + 1);
				int bigM = 800;
				...
				{
					int nnz = 4;
					int rhs = MIPUnicoParametros::MaxGapEntreFase_ + 2*bigM;
					...
					// Insere hip e hfp
					row.insert(colHip, 1);
					row.insert(colHfpAnt, -1);
					
					// Insere ptf e ptf
					row.insert(colPtf, bigM);
					row.insert(colPtfAnt, bigM);
...
int MIPUnico::criaRestricaoProfMinCredsDiaUsado()
{
	int restricoes = 0;
	...
	for ( ; itVarsProf != vars_prof_aula3.end(); itVarsProf++ )
	{
		...
		for ( ; itVarsDia != itVarsProf->second.end(); itVarsDia++ )
		{
			...
			auto finderDiaPT = finderProfPT->second.find(dia);
			if (finderDiaPT == finderProfPT->second.end()) continue; // error
			const int colPT = finderDiaPT->second;

			// Sets constraint
			ConstraintMIPUnico c;
			c.reset();
			c.setType(ConstraintMIPUnico::C_PROF_MIN_CREDS_DIA_USADO);
			c.setProfessor(professor);
			c.setDia(dia);
			...
			if(cit == cHashTatico.end())
			{
				...
				// pt
				int minCreds = professor->getMinCredsDiarios();
				if (minCreds <= 0) continue;

				row.insert(colPT, -minCreds);
