...
#ifndef ITERA_GGROUP_INIC_LESS
#define ITERA_GGROUP_INIC_LESS( it, it_inicio, ggroup, type ) for ( GGroup< type *, Less< type * > >::iterator it = it_inicio; it != ( ggroup ).end(); ++it )
#endif
...
#ifndef ITERA_GGROUP_LESS
#define ITERA_GGROUP_LESS( it, ggroup, type ) for ( GGroup< type *, Less< type * > >::iterator it = ( ggroup ).begin(); it != ( ggroup ).end(); ++it )
#endif
...
#ifndef ITERA_GGROUP_GREATER
#define ITERA_GGROUP_GREATER( it, ggroup, type ) for ( GGroup< type *, Greater< type * > >::iterator it = ( ggroup ).begin(); it != ( ggroup ).end(); ++it )
#endif
...
#ifndef ITERA_MAP_LESS
#define ITERA_MAP_LESS( it, mymap, type1, type2 ) for ( std::map< type1 *, type2, LessPtr< type1 * > >::iterator it = ( mymap ).begin(); it != ( mymap ).end(); ++it )
#endif
...
public:
	...
	virtual bool operator<(const OFBase& right) const { return (id < right.id); }
	virtual bool operator>(const OFBase& right) const {	return (id > right.id); }
