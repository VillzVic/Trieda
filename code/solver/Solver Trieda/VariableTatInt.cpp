...
VariableTatInt::VariableTatInt()
{
   reset();
}

VariableTatInt::VariableTatInt( const VariableTatInt & var )
{
   *this = var;
}

void VariableTatInt::reset()
{
   value = -1;
   i = -1;
   j = -1;
...
VariableTatInt & VariableTatInt::operator = ( const VariableTatInt & var )
{
...
bool VariableTatInt::operator < ( const VariableTatInt & var ) const
{
   ...
   if ( E_MENOR( this->getDateTimeInicial(), var.getDateTimeInicial() ) ) return true;
   if ( E_MENOR( var.getDateTimeInicial(), this->getDateTimeInicial() ) ) return false;

   if ( E_MENOR( this->getDateTimeFinal(), var.getDateTimeFinal() ) ) return true;
   if ( E_MENOR( var.getDateTimeFinal(), this->getDateTimeFinal() ) ) return false;
...
std::string VariableTatInt::toString()
{
   ...
   switch( type )
   {
	 ...
	 case V_SALA_PERIODO:
		str <<"us"; break;		
	 case V_FOLGA_UNID_PERIODO:
		str <<"fuu"; break;		
	 case V_FOLGA_SALA_PERIODO:
		str <<"fus"; break;		
