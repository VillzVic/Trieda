...
IteratedLocalSearchLevels::IteratedLocalSearchLevels(
   Avaliador & _e, Heuristic & _h, ILSLPerturbation & _p, int _iterMax, int _levelMax )
   : e( _e ), h( _h ), p( _p ), iterMax( _iterMax ), levelMax( _levelMax )
{
...
void IteratedLocalSearchLevels::localSearch(
   SolucaoOperacional & s, double timelimit, double target_f )
{
   h.exec( s, timelimit, target_f );
}

void IteratedLocalSearchLevels::perturbation(
   SolucaoOperacional & s, double timelimit, double target_f, levelHistory & history )
{
...
SolucaoOperacional & IteratedLocalSearchLevels::acceptanceCriterion(
   SolucaoOperacional & s1, SolucaoOperacional & s2, levelHistory & history )
{
...
void IteratedLocalSearchLevels::exec(
   SolucaoOperacional & s, double timelimit, double target_f )
{
   std::cout << "ILS exec(" << target_f << "," << timelimit << ")" << std::endl;
   ...
   std::cout << "ILS starts:\tSolution Evaluation: " << eStar << std::endl;
