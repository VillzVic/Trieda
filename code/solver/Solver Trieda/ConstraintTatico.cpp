...
{   
   ...
   this->dateTime = cons.getDateTime();
...
bool ConstraintTatico::operator < ( const ConstraintTatico & cons ) const
{
   ...
   if (E_MENOR(this->getDateTime(),cons.getDateTime())) return true;
   if (E_MENOR(cons.getDateTime(), this->getDateTime())) return false;
...
//      ss << "__(C_DEMANDA_DISC_ALUNO):"; break;
   ...
   case C_SALA_UNICA_TURMA:
      ss << "__(C_SALA_UNICA_TURMA):"; break;	  	  	      
   case C_AULA_PT_SEQUENCIAL:
      ss << "__(C_AULA_PT_SEQUENCIAL):"; break;	  	  	      
   case C_ALOC_MIN_ALUNO:
      ss << "__(C_ALOC_MIN_ALUNO):"; break;	  	  	      	  
   case C_DISC_PRATICA_TEORICA_1xN:
      ss << "__(C_DISC_PRATICA_TEORICA_1xN):"; break;	  	  	      	  
   ...
   if ( dateTime != NULL )
   {
      ss << "_dt" << dateTime->getHour() <<"_"<<dateTime->getMinute();
   }
