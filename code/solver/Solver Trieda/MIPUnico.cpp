...
void MIPUnico::addVariaveisTatico()
{
	...
	ITERA_GGROUP_LESSPTR( itVar, solVarsTatInt, VariableMIPUnico )
	{
		...
		{		
			 ...
			 case VariableMIPUnico::V_DIAS_CONSECUTIVOS: // c_{i,d,t,cp}
			 {
				VariableTatico * x = new VariableTatico();
			 ...
			 case VariableMIPUnico::V_SLACK_DIST_CRED_DIA_SUPERIOR: // fcp_{i,d,s,t}
			 {
				VariableTatico * x = new VariableTatico();
			 ...
			 case VariableMIPUnico::V_SLACK_DIST_CRED_DIA_INFERIOR: // fcm_{i,d,s,t}
			 {
				VariableTatico * x = new VariableTatico();
			 ...
			 case VariableMIPUnico::V_COMBINACAO_DIVISAO_CREDITO: // m{i,d,k}
			 {
				VariableTatico * x = new VariableTatico();
			 ...
			 case VariableMIPUnico::V_SLACK_COMBINACAO_DIVISAO_CREDITO_M: // fkm{i,d,t}
			 {
				VariableTatico * x = new VariableTatico();
			 ...
			 case VariableMIPUnico::V_SLACK_COMBINACAO_DIVISAO_CREDITO_P: // fkp{i,d,t}
			 {
				VariableTatico * x = new VariableTatico();
			 ...
			 case VariableMIPUnico::V_ABERTURA_COMPATIVEL: // zc_{d,t,cp}
			 {
				VariableTatico * x = new VariableTatico();
...
void MIPUnico::carregaVariaveisSolucao( int campusAtualId, int prioridade, int r )
{
   ...
   xSol = new double[ nroColsLP ];
	...
	if ( *(this->CARREGA_SOLUCAO) )
	{
		...
		if ( !status )
		{
		    std::cout << "\nErro em MIPUnico::carregaVariaveisSolucao(): arquivo "
					" nao encontrado.\n";
		    delete [] xSol;
			exit(0);
		}
...
//	if ( MIPUnico::idCounter > 1 || !SO_HEURN )
	if ( ! (*this->CARREGA_SOLUCAO) )
	{
		// Constraint creation
      ...
      //lp->setTimeLimit(1800);
		...
		cb_data.timeLimit = 3000;//this->getMaxTimeNoImprov(Solver::TAT_INT);
		cb_data.gapMax = 80;
		...
		// TRECHO IGUAL AO MAXATENDIMENTO!
   //      double *oldObj = new double[lp->getNumCols()];
   //      lp->getObj(0,lp->getNumCols()-1,oldObj);
   ...
   //         if ( v.getType() == VariableMIPUnico::V_SLACK_DEMANDA_ALUNO && xS[vit->second] < 0.001 )
   //         {
   //            lp->chgUB(vit->second,0.0);
   //         }

   //         lp->chgObj(vit->second,oldObj[vit->second]);
   //      }
...
void MIPUnico::polishTaticoHor(double *xSol, double maxTime, int percIni, int percMin, double maxTempoSemMelhora)
{
...
// ------------------------------------------------------------
   ...
   while (okIter)
   {
      ...
      while ( vit != vHashTatico.end() )
      {
         if ( vit->first.getType() == VariableMIPUnico::V_CREDITOS || vit->first.getType() == VariableMIPUnico::V_ABERTURA )
         {
            idxs[nBds] = vit->second;
...
GGroup< VariableMIPUnico *, LessPtr<VariableMIPUnico> > MIPUnico::retornaAulasEmVarXFinal( int turma, Disciplina* disciplina, int campusId )
{
	GGroup< VariableMIPUnico *, LessPtr<VariableMIPUnico> > aulasX;

	ITERA_GGROUP_LESSPTR( itSol, this->solVarsTatInt, VariableMIPUnico )
	{
		VariableMIPUnico *x = *itSol;
...
GGroup< VariableMIPUnico *, LessPtr<VariableMIPUnico> > 
	MIPUnico::retornaAulasEmVarV( Aluno* aluno, GGroup< VariableMIPUnico *, LessPtr<VariableMIPUnico> > aulasX )
{
	GGroup< VariableMIPUnico *, LessPtr<VariableMIPUnico> > aulasV;

	ITERA_GGROUP_LESSPTR( itSol, this->solVarsTatInt, VariableMIPUnico )
	{
		VariableMIPUnico *v = *itSol;
		if ( v->getType() == VariableMIPUnico::V_ALUNO_CREDITOS )
		{
			if ( v->getAluno() == aluno )
			{
				ITERA_GGROUP_LESSPTR( itX, aulasX, VariableMIPUnico )
				{
...
{	
	...
	switch( v->getType() )
	{
		 ...
		 case VariableMIPUnico::V_OFERECIMENTO:  //  o_{i,d,u,s} 
		 {
		 ...
		 case VariableMIPUnico::V_ALOCA_ALUNO_TURMA_DISC:  //  s_{i,d,a} 
		 {
		 ...
		 case VariableMIPUnico::V_DIAS_CONSECUTIVOS: // c_{i,d,t}
		 {
			 vSol.reset();
...
int MIPUnico::criaVariavelTaticoAlunoCreditosAPartirDeX_MaisFiltroAluno( int campusId, int P )
{
	...
	std::map< Disciplina*, std::map< ConjuntoSala*, std::map< int/*dia*/, map<int/*id*/, VariableMIPUnico> >,
		LessPtr<ConjuntoSala> >, LessPtr<Disciplina> >::iterator itDisc;

	std::map< ConjuntoSala*, std::map< int/*dia*/, map<int/*id*/, VariableMIPUnico> >,
		LessPtr<ConjuntoSala> >::iterator itCjtSala;

	std::map< int/*dia*/, map<int/*id*/, VariableMIPUnico> >::iterator itDia;

	map< int /*id*/, VariableMIPUnico >::iterator itX;
		...
		{	
			...
			ITERA_GGROUP_LESSPTR ( itAlDem, (*alunosDemanda), AlunoDemanda )
			{
				...
				for ( itCjtSala = (*mapCjtSala).begin(); itCjtSala != (*mapCjtSala).end(); itCjtSala++ )
				{
					...
					std::map< int/*dia*/, map<int/*id*/, VariableMIPUnico> > *mapDia = & itCjtSala->second;
					...
					{
						int dia = itDia->first;
						map< int /*id*/, VariableMIPUnico > *mapX = & itDia->second;
				...
				//				std::cout << "\n\tValida realmente!";
					...
					{
						map< int/*dia*/, vector<VariableMIPUnico> >::iterator itDiaV = varsDiscAluno.begin();
...
// x_{i,d,u,s,hi,hf,t}
int MIPUnico::criaVariavelTaticoCreditos__( int campusId, int P )
{
	int numVars = 0;
	...
	{		
		...
		Disciplina* disciplina = x.getDisciplina();
		ConjuntoSala *cjtSala = x.getSubCjtSala();
		Campus *campus = problemData->refCampus[ cjtSala->getCampusId() ];
		int dia = x.getDia();
...
// o_{i,d,u,s}
int MIPUnico::criaVariavelTaticoOferecimentos( int campusId, int P )
{
	int numVars = 0;
...
// s_{i,d,a,cp}
int MIPUnico::criaVariavelTaticoAlocaAlunoTurmaDisc( int campusId, int P )
{
	int numVars = 0;
...
// c_{i,d,t,cp}
int MIPUnico::criaVariavelTaticoConsecutivosAPartirDeX( int campusId, int P )
{
	int numVars = 0;
...
%DocEnd
/====================================================================*/
int MIPUnico::criaVariavelTaticoConsecutivos( int campusId, int P )
{
	int numVars = 0;
...
int MIPUnico::criaVariavelTaticoFolgaDistCredDiaSuperior( int campusId, int P )
{
   int numVars = 0;
                  ...
                  for (; itDiasLetDisc != dias_letivos.end(); itDiasLetDisc++ )
                  {
                     ITERA_GGROUP_LESSPTR( it_fix, problemData->fixacoes, Fixacao )
                     {
                           ...
                           && it_fix->getDiaSemana() == ( *itDiasLetDisc ) )
                        {
                           VariableMIPUnico v;
                           v.reset();
                           v.setType( VariableMIPUnico::V_SLACK_DIST_CRED_DIA_SUPERIOR );
...
int MIPUnico::criaVariavelTaticoFolgaDistCredDiaInferior( int campusId, int P )
{
   int numVars = 0;
                  ...
                  for (; itDiasLetDisc != dias_letivos.end(); itDiasLetDisc++ )
                  {
                     ITERA_GGROUP_LESSPTR( it_fix, problemData->fixacoes, Fixacao )
                     {
                           ...
                           && it_fix->getDiaSemana() == ( *itDiasLetDisc ) )
                        {
                           VariableMIPUnico v;
                           v.reset();
                           v.setType( VariableMIPUnico::V_SLACK_DIST_CRED_DIA_INFERIOR );
...
%DocEnd
/====================================================================*/
int MIPUnico::criaVariavelTaticoAberturaCompativel( int campusId, int P )
{
   int numVars = 0;
...
// s_{i,d,a,cp}
int MIPUnico::criaVariavelTaticoAlocaAlunoTurmaDiscEquiv( int campusId, int P )
{
	int numVars = 0;
...
// s_{i,d,a,cp}
int MIPUnico::criaVariavelTaticoAlocaAlunoTurmaDiscEquivTotal( int campusId, int P )
{
...
// z_{i,d,cp}
int MIPUnico::criaVariavelTaticoAbertura( int campusId, int prior, int r )
{
	int numVars = 0;
					...
					for ( auto itDia = itTurma->second.begin(); itDia != itTurma->second.end(); itDia++ )
					{
						for ( auto itDti = itDia->second.begin(); itDti != itDia->second.end(); itDti++ )
						{
							for ( auto itVar = itDti->second.begin(); itVar != itDti->second.end(); itVar++ )
							{
								...
								Campus * const campus = problemData->refCampus[ x.getSubCjtSala()->getCampusId() ];
								...
								auto *map1 = & vars_prof_aula1[campus][disciplina][turma][professor][dia];
								...
								v.setHorarioAulaInicial(h);
									...
									{					
										...
										(*map2)[xDti][campus][disciplina][turma] = make_pair(colNr,v);
...
//   timer.start();
//	restricoes += criaRestricaoGapsHorariosProfessores();	// Restricao 1.2.37
//	timer.stop();
//	dif = timer.getCronoCurrSecs();
//
//#ifdef PRINT_cria_restricoes
//	std::cout << "numRest criaRestricaoGapsHorariosProfessores: " << (restricoes - numRestAnterior)  <<" "<<dif <<" sec" << std::endl;
//	numRestAnterior = restricoes;
//#endif
...
*/
int MIPUnico::criaRestricaoTaticoAtendeAlunoEquiv( int campusId, int prioridade )
{
    int restricoes = 0;
...
/*
	...
	sum[i]sum[d]sum[dti]sum[dtf] k_{p,i,d,t,dti,dtf} <= 1
			...
			{				
				...
				if(cit == cHashTatico.end())
				{
					...
					for ( ; itVarsDti != itVarsDia->second.end(); itVarsDti++ )
					{
						...
						// Vars
						auto itVarsCp = itVarsDti->second.begin();
						for ( ; itVarsCp != itVarsDti->second.end(); itVarsCp++ )
						{
...
/*
	...
	Para cada turma i, disciplina d, campus cp, professor p, dia t, datetime dti:

	sum[hi] sum[hf] sum[s] x_{i,d,s,t,hi,hf} <= k_{p,i,d,cp,t,dti} + ( 1 - y_{p,i,d,cp} )
...
int MIPUnico::criaRestricaoProfAula()
{
	...
	// Campus
	auto itVarsCp = vars_prof_aula1.begin();
	for ( ; itVarsCp != vars_prof_aula1.end(); itVarsCp++ )
	{
		Campus *campus = itVarsCp->first;
		
		auto itVarCpX = vars_turma_aula.find(campus);
		if ( itVarCpX == vars_turma_aula.end() ) continue;
		...
		for ( ; itVarsDisc != itVarsCp->second.end(); itVarsDisc++ )
		{
			...
			auto itVarDiscX = itVarCpX->second.find(disciplina);
			if ( itVarDiscX == itVarCpX->second.end() ) continue;
...
/*
	...
	Para cada turma i, disciplina d, campus cp, dia t, datetime dti:

	sum[hi] sum[hf] sum[s] x_{i,d,s,t,hi,hf} = sum[p] k_{p,i,d,cp,t,dti}
...
/*
	...
	sum[t] sum[h] k_{p,i,d,cp,t,h} = nCreds_{d} * y_{p,i,d,cp}
			...
			for ( ; itVarsTurma != itVarsDisc->second.end(); itVarsTurma++ )
			{
				...
				for ( ; itVarsProf != itVarsTurma->second.end(); itVarsProf++ )
				{
					...
					if(cit == cHashTatico.end())
					{
						...
						// -------------------------------------
						// Insere variaveis k_{p,i,d,cp,t,dti}
