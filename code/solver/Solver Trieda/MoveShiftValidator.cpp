...
MoveShiftValidator::MoveShiftValidator( ProblemData * pD )
   : MoveValidator( pD )
{
...
bool MoveShiftValidator::canShiftSchedule(
   Aula & aula, Professor & teacher, std::vector< HorarioAula * > newSchedule,
   SolucaoOperacional & solOp, std::vector< std::string > checksToDo )
{
   ...
   {    
      ...
      {
         if ( !totalSchedules( itBlocks, newSchedule ) )
         {
            return false;
         }
      }
      ...
      {
         if ( conflictScheduleTeachers( teacher, itBlocks, newSchedule ) )
         {
            return false;
         }
      }
      ...
      {
         if ( !newScheduleFree( aula, teacher, newSchedule, solOp ) )
         {
            return false;
         }
      }
      ...
      {
         if ( checkBlockConflict( aula, newSchedule, solOp ) )
         {
            return false;
         }
      }
      ...
      {
         if ( !checkClassAndLessonDisponibility( aula, newSchedule, solOp ) )
         {
            return false;
         }
      }
...
cItClassesBlocks MoveShiftValidator::getClassesBlocks( Aula & aula, SolucaoOperacional & solOp )
{
   ...
   {
      std::cout << "Nao encontrei a aula desejada na estrutura "
                << "de blocoAula de uma dada solucao. ERRO.\n";
...
{
   // Checando se a qtd de horários a serem alocados
   // são iguais (em quantidade) aos horários da aula em questão.
...
bool MoveShiftValidator::conflictScheduleTeachers(
   Professor & teacher, cItClassesBlocks & itBlocks, std::vector< HorarioAula * > & newSchedule )
{
   // Checando se o professor corrente é o mesmo que o novo professor.
   if ( sameTeachers( teacher, itBlocks ) )
   {
      // Se, pelo menos, um dos novos horários for comum aos horários antigos
      // da aula em questão deve-se retornar false. Ou seja, o shift não será realizado.

      std::vector< HorarioAula * >::iterator
         itNovosHorariosAula = newSchedule.begin();

      // Para cada novo horário da aula em questão, checo se ele
      // conflita com  algum horário da aula selecionada do bloco curricular.
...
bool MoveShiftValidator::newScheduleFree(
   Aula & aula, Professor & teacher, std::vector< HorarioAula * > & newSchedule, SolucaoOperacional & solOp )
{
   // Verificando se os possíveis novos horarios estão desocupados (NULL) na solução.
   for ( int h = 0; h < (int)newSchedule.size(); ++h )
   {
      ...
      {
         if ( ( *itHorariosAulaOrd )->getId() == newSchedule.at( h )->getId() )
         {
            break;
         }
      }
      ...
      {
         std::cout << "ERRO em <bool MoveShiftValidator::canShiftSchedule(Aula & aula, "
                   << "std::vector< std::pair< Professor*, Horario* > > blocoHorariosVagos) const>. "
