...
IS_Bloco_Curricular::IS_Bloco_Curricular( BlocoCurricular * blc )
{
   ...
   std::map< Disciplina *, Demanda * >::iterator 
      it_Disciplina_Demanda = bloco->disciplina_Demanda.begin();

   for (; it_Disciplina_Demanda != bloco->disciplina_Demanda.end();
          it_Disciplina_Demanda++ )
   {
      disciplina_Dem_Nao_Atendida[ it_Disciplina_Demanda->first ]
         = it_Disciplina_Demanda->second->getQuantidade();
...
{
   std::cerr << "COPY CONSTRUCTOR OF <IS_Bloco_Curricular> NOT IMPLEMENTED YET" << std::endl;
   exit(1);
}
