...
\begin{eqnarray} 
b_{i,d,c} + b_{i,d,c'} - bs_{i,d,c,c'} \leq 1 \nonumber \qquad 
\forall d \in D \quad
\forall i \in I_{d} \quad
...
\item $Nh_{sl,kh,s,t}$ - número máximo de horários da semana letiva $sl$ determinados para a sala $s$ no dia $t$ na combinação de divisão de horários $kh$.
...
\subsubsection{Evita sobreposição de horário em sala devido a compartilhamento de disciplinas entre dois cursos}
...
\begin{eqnarray}	
\sum\limits_{d \in D_{bc}} ( g_{d,oft,t} ) \le MaxCred_{oft,t} \nonumber \qquad
