...
\begin{array}{rll}
   \mbox{MIN} &
      ...
      \\           
      &
      + \sum\limits_{cp \in CP} \sum\limits_{d \in D} da_{d,cp}
      \\
...
\begin{eqnarray}
\sum\limits_{g \in G} q_{i,d,s,g} = o_{i,d,s}  \nonumber \qquad 
\forall i \in I \quad
\forall d \in D \quad
\forall s \in S
\end{eqnarray}
...
% Não precisa, se for usar a associação de "q" com "o" como igualdade
...
\subsubsection{Distribuição equilibrada de alunos nas turmas de mesma disciplina}
\begin{eqnarray}
\sum\limits_{s \in S} \sum\limits_{oft \in Oft} a_{i,d,s,oft} \le da_{d,cp} \nonumber \qquad 
\forall cp \in CP \quad
\forall d \in D \quad
\forall i \in I_{d}
\end{eqnarray}
