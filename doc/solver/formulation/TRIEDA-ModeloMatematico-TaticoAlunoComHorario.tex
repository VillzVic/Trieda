...
\begin{array}{rll}
   \mbox{MAX} &
      ...
      \\
      &
       - \sum\limits_{i \in I_{d}}\sum\limits_{d \in D_{oft}}
      \sum\limits_{u \in U}\sum\limits_{s \in S_{u}} \sum\limits_{oft \in Oft} c_{i,d,u,s,oft}
      \\
      &
      - 100 \cdot \sum\limits_{i \in I_{d}}\sum\limits_{d \in D_{c} \cap D_{c'}}
...
\item $fd_{d,oft}$ - variável de folga, indica que a turma $i$ da disciplina $d$ no campus $cp$ não foi atendida.
...
\begin{eqnarray}
\sum\limits_{u \in U_{cp}} \sum\limits_{s \in S_{u}} \sum\limits_{i \in I_{d,a}} \sum\limits_{d \in D_{a}} \sum\limits_{hi \in H_{d}} \sum_{\substack {hf \in H_{d} \\ hi\le hf \\ (hi,hf)\mbox{ sobrepoe }h}} x_{i,d,u,s,hi,hf,t}  \leq  1  \nonumber \qquad 
\forall cp \in CP \quad
\forall a \in A \quad
\forall t \in T \quad
\forall h \in H_{d}
...
\begin{eqnarray}
\sum\limits_{u \in U_{cp}} \sum\limits_{s \in S_{u}} \sum\limits_{i \in I_{d}} \sum\limits_{d \in D} \sum\limits_{hi \in H_{d}} \sum_{\substack {hf \in H_{d} \\ hi\le hf \\ (hi,hf)\text{ sobrepoe }h}} x_{i,d,u,s,hi,hf,t} 
+
\sum\limits_{u \in U_{cp}} \sum\limits_{s \in S_{u}} \sum\limits_{i' \in I_{d'}} \sum\limits_{d' \in D} \sum\limits_{hi \in H_{d}} \sum_{\substack {hf \in H_{d} \\ hi\le hf \\ (hi,hf)\text{ sobrepoe }h}} x_{i',d',u,s,hi,hf,t} 
\leq  1  \nonumber \qquad 
\\
