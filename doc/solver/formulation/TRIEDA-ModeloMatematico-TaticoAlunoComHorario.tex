...
\item $xcs2$
\item $xcs3$
\item $v_{i,d,sl}$ - variável binaria que indica se a turma $i$ da disciplina $d$ possui alunos pertencentes ao calendario $sl$.
...
\begin{array}{rll}
   \mbox{MIN} &
      ...
      \\
      &        
      + 50 \cdot \sum\limits_{s \in S} xcs2_{s}
      \\
      &        
      + 1000 \cdot \sum\limits_{s \in S} xcs3_{s}
...
\begin{array}{rll}
   \mbox{MAX} &
      ...
      \\
      &     
      + 10 \cdot \sum\limits_{oft \in O} \sum\limits_{d \in D_{oft}}\sum\limits_{i \in I_{d}}
      						\sum\limits_{s \in S} nCreds_{d} \cdot r_{oft} \cdot a_{i,d,s,oft}
      \\
      &  
      - 100 \cdot \sum\limits_{a \in Al} fps_{a}
      \\
...
%
%\begin{eqnarray}
%\sum\limits_{d \in D}\sum\limits_{i \in I_{d}} tempo_{d} \cdot x_{i,d,u,s}  \leq  HTPS_{s}  \nonumber \qquad 
%\forall u \in U \quad
%\forall s \in S_{u} \quad
%\end{eqnarray}
...
\begin{eqnarray}
\sum\limits_{d \in D}\sum\limits_{i \in I_{d}} tempo_{d} \cdot C_{d} \cdot o_{i,d,u,s}  \leq  HTPS_{s}  \nonumber \qquad 
\forall u \in U \quad
\forall s \in S_{u} \quad
\end{eqnarray}
...
% Pode tirar se for retirar a_{i,d,s,oft} do modelo, e deixar só a restrição correspondente a essa abaixo!
%
...
\begin{eqnarray}
\sum\limits_{a \in A} s_{i,d,a} \leq (Cap_{s} - BigM) \cdot o_{i,d,u,s} + BigM
\nonumber \qquad 
\forall u \in U \quad
\forall s \in S_{u} \quad
\forall d \in D \quad
...
%
%
%\subsubsection{Ativa variável z}
%
%\begin{eqnarray}
...
\begin{eqnarray}
MinAlunos \cdot (z_{i,d,cp} - f_{i,d,cp}) \leq \sum\limits_{a \in A} s_{i,d,a}  \nonumber \qquad 
\forall cp \in CP \quad
\forall d \in D \quad
\forall i \in I_{d}
\end{eqnarray}
...
% Pode tirar se for retirar a_{i,d,s,oft} do modelo, e acrescentar a restrição abaixo
%\subsubsection{Atendimento de alunos}
%
%\begin{eqnarray}
%\sum\limits_{s \in S} a_{i,d,s,oft} = \sum\limits_{a \in Al \cap d \in D_{a} } s_{i,d,a} \nonumber \qquad 
%\forall d \in D_{oft} \quad
%\forall i \in I_{d} \quad
%\forall oft \in O \quad
%\end{eqnarray}

\subsubsection{Só permite alocação de aluno na turma caso essa tenha sido aberta}
\begin{eqnarray}
\sum\limits_{a \in A} s_{i,d,a} \le MaxAlunos \cdot z_{i,d,cp} \nonumber \qquad 
\forall d \in D_{oft} \quad
\forall i \in I_{d} \quad
\forall oft \in O \quad
\end{eqnarray}
...
%\subsubsection{Evita sobreposição de turmas com mesmos alunos para um par de salas: só para rodada de prioridade 2 em diante e tático de P2 fixado}
%
%\begin{eqnarray}
%tempo_{i,d,t} \cdot z_{i,d,cp} + \sum\limits_{i' \in I_{d'} \cap i' \in I_{d',s',t}}\sum\limits_{d' \in D} tempo_{i',d',t}\cdot w_{i',d',i,d} \le %MaxTempo \nonumber \qquad 
%\forall \mbox{ alocação já existente } (i,d,s) \quad
%\forall t \in T \quad
...
\begin{eqnarray}
s_{i,d,a} + s_{i',d',a} - w_{i,d,i',d'} \le 1 \nonumber \qquad 
\forall a \in Al \quad
...
\begin{eqnarray}
\sum\limits_{i \in I_{d}}\sum\limits_{d \in D} nCreds_{i,d,t} \cdot s_{i,d,a} \le NroMaxCreds_{a,t,sl} \nonumber \qquad 
\forall a \in Al \quad
\forall t \in T \quad
\forall sl \in SL
\end{eqnarray}
...
\begin{eqnarray}
\sum\limits_{i \in I_{d}}\sum\limits_{d \in D} tempo_{i,d,t} \cdot s_{i,d,a} \le MaxTempo_{a,t} \nonumber \qquad 
\forall a \in Al \quad
\forall t \in T
\end{eqnarray}
...
\begin{eqnarray}
\sum\limits_{i \in I_{d}} \sum\limits_{d \in D} tempo_{d} \cdot C_{d} \cdot o_{i,d,s} = xcs1_{s} + xcs2_{s} + xcs3_{s} \nonumber \qquad 
\forall s \in S \quad
\end{eqnarray}
...
\begin{eqnarray}
as_{a,s} \ge o_{i,d,s} + s_{i,d,a} - 1 \nonumber \qquad 
\forall a \in Al \quad
\forall s \in S \quad
\forall i \in I_{d} \quad
\forall d \in D_{a}
...
\begin{eqnarray}
M \cdot v_{i,d,sl} \ge \sum\limits_{a \in A_{sl}} s_{i,d,a} \nonumber \qquad 
...
\begin{eqnarray}
v_{i,d,sl} \le \sum\limits_{a \in A_{sl}} s_{i,d,a} \nonumber \qquad 
\forall sl \in SL \quad
\forall i \in I_{d} \quad
\forall d \in D
\end{eqnarray}
...
\subsubsection{Garante que caso uma mesma turma possua alunos de diferentes semanas letivas alocados, estas devem possuir horários em comum}

\begin{eqnarray}
v_{i,d,sl1} + v_{i,d,sl2} \le 1 \nonumber \qquad 
...
\begin{eqnarray}
fd_{i1,d,cp} + fd_{i2,-d,cp} - 2 \cdot ffd_{i1,d,i2,-d,cp} \le 0  \nonumber \qquad 
