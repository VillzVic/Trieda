...
\subsubsection{Impede atendimento parcial (Só necessária quando existe folga de demanda $fd_{a}$) }

\begin{eqnarray}
\sum\limits_{a \in A} fd_{a} = nroAulas_{a} \cdot ( 1 - \sum\limits_{p \in P \cap PV} y_{p,i,d,cp} ) \nonumber \qquad 
\forall d \in D \quad
...
\subsubsection{Somente um professor deve ser alocado para duas turmas de disciplina prática e teórica com aluno em comum}
...
\begin{eqnarray}
\sum\limits_{p \in P} \sum\limits_{pv \in PV} \sum\limits_{s \in S_{cp}}  x_{p,a,s,d,i,t,h} + x_{pv,a,s,d,i,t,h} \leq z_{d,i,h,cp} \nonumber \qquad 
\forall cp \in Cp \quad
\forall d \in D \quad
\forall i \in I_{d} \quad
\forall t \in T \quad
...
\subsubsection{Estima o número de professores virtuais no curso (Somente para primeira rodada, quando professores virtuais estão sendo estimados)}
...
\subsubsection{Estima o número de professores virtuais mestres no curso (Somente para primeira rodada, quando professores virtuais estão sendo estimados)}
...
\subsubsection{Estima o número de professores virtuais doutores no curso (Somente para primeira rodada, quando professores virtuais estão sendo estimados)}
...
\subsubsection{Estima o número de professores virtuais gerais no curso (Somente para primeira rodada, quando professores virtuais estão sendo estimados)}
...
\subsubsection{Calcula o número de professores virtuais no curso ((Somente para segunda rodada, quando professores virtuais já foram criados)}

\begin{eqnarray}
\sum\limits_{p \in PV} w_{p,c} = npv_{c} \nonumber \qquad 
\forall c \in C
\end{eqnarray}


\subsubsection{Calcula o número de professores virtuais mestres no curso (Somente para segunda rodada, quando professores virtuais já foram criados)}

\begin{eqnarray}
\sum\limits_{p \in PV_{m}} w_{p,c} = npvm_{c} \nonumber \qquad 
\forall c \in C
\end{eqnarray}


\subsubsection{Calcula o número de professores virtuais doutores no curso (Somente para segunda rodada, quando professores virtuais já foram criados)}

\begin{eqnarray}
\sum\limits_{p \in PV_{d}} w_{p,c} = npvd_{c} \nonumber \qquad 
\forall c \in C
\end{eqnarray}


\subsubsection{Calcula o número de professores virtuais gerais no curso (Somente para segunda rodada, quando professores virtuais já foram criados)}

\begin{eqnarray}
\sum\limits_{p \in PV_{g}} w_{p,c} = npvg_{c} \nonumber \qquad 
\forall c \in C
\end{eqnarray}
      

\subsubsection{Uso de professor virtual (Somente para segunda rodada, quando professores virtuais já foram criados) }
...
\subsubsection{Garante número mínimo de professores no curso (Somente para Rodada 1). É necessária devido à restrição de máximo de disciplinas ministradas por um professor por curso.}

\begin{eqnarray}
MaxDiscProfCurso_{c} \cdot (np_{c} + npv_{c}) \ge qtdDiscComAulas_{c} \nonumber \qquad 
\forall c \in C \quad
\end{eqnarray}
...
\begin{eqnarray}
hfp_{p,t} \ge m(h) \cdot \sum\limits_{a \in A_{t}} x_{p,a,h} \nonumber \qquad 
\forall p \in P \quad
\forall t \in T \quad
\forall h \in H
\end{eqnarray}
...
% Não deve precisar disso, já que o operacional tem que atender tudo, ainda que use prof virtual.
%\subsubsection{Alocação minima de demanda por aluno}
%
%\begin{eqnarray}
%\sum\limits_{ \substack{ d \in D \\ a \in (i,d,cp) } } nCreds_{d} \cdot \sum\limits_{p \in P} y_{p,d,i,cp} \ge MinAtendPerc \cdot TotalDemanda_{a} - fmd_{a} \nonumber \qquad 
%\forall a \in Al \quad
%\end{eqnarray}
