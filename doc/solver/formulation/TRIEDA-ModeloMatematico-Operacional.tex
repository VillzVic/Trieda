...
\item $x_{p,a,s,d,i,t,h}$ - variável binária indicando que o professor $p$ foi alocado a aula $a$ no horário $h$.
\item $x_{pv,a,s,d,i,t,h}$ - variável binária indicando que o professor virtual $pv$ foi alocado a aula $a$ no horário $h$.
...
\item $fdh_{d,i,t,h}$ - variável de folga para a restrição de um horário $h$ no dia $t$ para a turma $i$ da disciplina $d$. 
...
\begin{array}{rll}
   \mbox{MIN} &
      ...
      \\
      &
       + \gamma \cdot \sum\limits_{d \in D} \sum\limits_{i \in I_{d}} \sum\limits_{t \in T} \sum\limits_{h \in H} fdh_{d,i,t,h}
      \\
\end{array}
$$
...
\begin{eqnarray}
\sum\limits_{p \in P}\sum\limits_{pv \in PV}\sum\limits_{h \in H} x_{p,a,s,d,i,t,h} + x_{pv,a,s,d,i,t,h}  =  1 \nonumber \qquad 
\forall a \in A 
\end{eqnarray}
...
\begin{eqnarray}
\sum\limits_{p \in P} \sum\limits_{pv \in PV} \sum\limits_{a \in A} x_{p,a,s,d,i,t,h} + x_{pv,a,s,d,i,t,h} \leq  1 \nonumber \qquad 
\forall s \in S \quad
\forall t \in T \quad
\forall h \in H
\end{eqnarray}
...
\begin{eqnarray}
\sum\limits_{s \in S} \sum\limits_{a \in A} x_{p,a,s,d,i,t,h} \leq  1 \nonumber \qquad 
\forall p \in P \quad
\forall t \in T \quad
\forall h \in H
\end{eqnarray}

\subsubsection{Aulas de mesmo bloco para cada horário}
...
\subsubsection{Associação das variáveis x e y}
...
\subsubsection{Um mesmo professor tem que ser alocado para aulas de mesma disciplina e turma}
...
\begin{eqnarray}
\sum\limits_{p \in P} \sum\limits_{pv \in PV} \sum\limits_{s \in S}  x_{p,a,s,d,i,t,h} + x_{pv,a,s,d,i,t,h} \leq z_{d,i,h} + fdh_{d,i,t,h} \nonumber \qquad 
\forall d \in D \quad
...
\begin{eqnarray}
\sum\limits_{h \in H} z_{d,i,h} \leq 1 \nonumber \qquad 
\forall d \in D \quad
\forall i \in I_{d}
\end{eqnarray}
