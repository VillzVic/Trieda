...
\item $Per_{oft}$ - Conjunto de períodos da matriz curricular associada à oferta $oft$. Os elementos desse conjunto são 
denotados por $p$.
...
\item $delta_{a,t}$ - tempo máximo de gap permitido dentro do dia t para o aluno a. É basicamente a soma da duração dos intervalos do dia.

\item $delta_{t,f}$ - tempo máximo de gap permitido dentro da fase do dia f do dia t para um professor. É basicamente a soma da duração dos intervalos da fase do dia $f$ no dia $t$.
...
\item $z_{i,d,cp}$ - binária que indica se a turma $i$ da disciplina $d$ no campus $cp$ foi aberta. 
...
\item $fp_{d,t,h}$ - variável de folga inteira que indica se a disciplina $d$ tem aula no dia $t$ e horario $h$ e não possui professor real com disponibilidade nesse dia e horário para ministra-la.
...
\item $uu_{u,oft,p}$ - variável binária de uso de unidade por período de oferta. Indica que a unidade $u$ está sendo usada por alguma turma com alunos do período $p$ da oferta $oft$.
\item $fuu_{oft,p}$ - variável inteira de folga de única unidade. Quando é usada, o seu valor indica o excesso de unidades usadas pelo período $p$ da oferta $oft$ que extrapola a exigência de unidade única para o período.
\item $us_{s,oft,p}$ - variável binária de uso de sala por período de oferta. Indica que a sala $s$ está sendo usada por alguma turma com alunos do período $p$ da oferta $oft$.
\item $fus_{oft,p}$ - variável inteira de folga de única sala. Quando é usada, o seu valor indica o excesso de salas usadas pelo período $p$ da oferta $oft$ que extrapola a exigência de sala única para o período.
\item $k_{p,i,d,hi,hf,t}$ - indica se o professor $p$ dá aula para a turma $i$ da disciplina $d$ do horário $hi$ ao horário $hf$ do dia $t$. 
...
\item $inicio_{a,t,h}$ - variável binária que indica se as aulas do aluno $a$ no dia $t$ se iniciam no horario $h$.
\item $fim_{a,t,h}$ - variável binária que indica se as aulas do aluno $a$ no dia $t$ terminam no horario $h$.
\item $at_{a,t}$ - variável binária que indica se o aluno $a$ tem aula no dia $t$.
...
\begin{eqnarray}
\sum\limits_{d \in D} nCreds_{d} (1 - fd_{d,a}) \ge MinAtendPerc \cdot TotalDemanda_{a} - fmd_{a} \nonumber \qquad 
\forall a \in Al
\end{eqnarray}
...
\begin{eqnarray}
\sum\limits_{s \in S} Cap_{s} \cdot o_{i,d,s} - \sum\limits_{a \in Al} s_{i,d,cp,a} = fos_{i,d,cp} \nonumber \qquad 
\forall i \in I \qquad
\forall d \in D \qquad
\forall cp \in CP
\end{eqnarray}
...
\subsubsection{Seta o uso da sala para cada periodo de oferta}

\begin{eqnarray}
\sum\limits_{a \in A}\sum\limits_{i \in I_{d}}\sum\limits_{d \in D}\sum\limits_{u \in U}\sum\limits_{t \in T}\sum\limits_{hi \in H}\sum\limits_{hf \in H} v_{a,i,d,s,u,t,hi,hf} \le bigM \cdot us_{s,oft,p} \nonumber \qquad \\
\mbox{ aonde a = aluno cujo AlunoDemanda (a,d) esteja associado ao período p da oferta oft} \qquad \\
\forall oft \in O \qquad
\forall p \in Per_{oft} \qquad
\forall s \in S
\end{eqnarray}



\subsubsection{Seta o uso da unidade para cada periodo de oferta}

\begin{eqnarray}
\sum\limits_{a \in A}\sum\limits_{i \in I_{d}}\sum\limits_{d \in D}\sum\limits_{s \in S}\sum\limits_{t \in T}\sum\limits_{hi \in H}\sum\limits_{hf \in H} v_{a,i,d,s,u,t,hi,hf} \le bigM \cdot uu_{u,oft,p} \nonumber \qquad \\
\mbox{ aonde a = aluno cujo AlunoDemanda (a,d) esteja associado ao período p da oferta oft} \qquad \\
\forall oft \in O \qquad
\forall p \in Per_{oft} \qquad
\forall u \in U
...
\subsubsection{Limita as unidades usadas por cada período}

\begin{eqnarray}
\sum\limits_{u \in U} uu_{u,oft,p} + fuu_{oft,p} \le 1 \nonumber \qquad 
\forall oft \in O \qquad
\forall p \in Per_{oft}
\\ \mbox{ sendo que a folga só existe caso haja possibilidade de compartilhamento de aulas do período com outra oferta.}
\end{eqnarray}


	
\subsubsection{Limita as salas usadas por cada período}

\begin{eqnarray}
\sum\limits_{s \in S} us_{s,oft,p} + fus_{oft,p} \le 1 \nonumber \qquad 
\forall oft \in O \qquad
\forall p \in Per_{oft}
\\ \mbox{ sendo que a folga só existe caso haja possibilidade de compartilhamento de aulas do período com outra oferta.}
\end{eqnarray}
...
\subsubsection{Evita sobreposição de horários de professores em um mesmo dia}
...
\subsubsection{Associa professor à aula (seta variável k)}
...
\subsubsection{Associa professor à turma (seta variável y)}
...
\subsubsection{Associa único professor à turma}
...
\subsubsection{Contabiliza se o aluno possui aula no dia}
...
\subsubsection{Seta o inicio das aulas do aluno no dia ($inicio_{a,t,h}$)}

\begin{eqnarray}
inicio_{a,t,h} = \sum\limits_{v} v_{a,t,hi} - \sum\limits_{h'\le h} inicio_{a,t,h'} + \sum\limits_{h'\le h} fim_{a,t,h'} \nonumber \qquad
\forall a \in A \quad
\forall t \in T \quad
\forall h \in H
\end{eqnarray}
...
\subsubsection{Seta o fim das aulas do aluno no dia ($fim_{a,t,h}$)}

\begin{eqnarray}
fim_{a,t,h} = \sum\limits_{v} v_{a,t,hi} - \sum\limits_{h'\ge h} fim_{a,t,h'} + \sum\limits_{h'\ge h} inicio_{a,t,h'} \nonumber \qquad
\forall a \in A \quad
\forall t \in T \quad
\forall h \in H
\end{eqnarray}
...
\subsubsection{Unicidade de fins por dia}

\begin{eqnarray}
\sum\limits_{h} fim_{a,t,h} = at_{a,t} \nonumber \qquad
\forall a \in A \quad
\forall t \in T
\end{eqnarray}


\subsubsection{Unicidade de inicios por dia}

\begin{eqnarray}
\sum\limits_{h} inicio_{a,t,h} = at_{a,t} \nonumber \qquad
\forall a \in A \quad
\forall t \in T
\end{eqnarray}


\subsubsection{Proíbe gap no horário do aluno em um mesmo dia}

\begin{eqnarray}
\sum\limits_{i} \sum\limits_{d} \sum\limits_{s} \sum\limits_{hf} v_{a,i,d,s,t,hi+,hf} = 
\sum\limits_{i} \sum\limits_{d} \sum\limits_{s} \sum\limits_{hf} v_{a,i,d,s,t,hi-,hf} - fim_{a,t,hi-} + inicio_{a,t,hi+} \nonumber \qquad
\forall a \in A \quad
\forall t \in T \quad
