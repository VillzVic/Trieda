...
\item $fs_{d,s,g}$ - número de turmas excedentes a 1 da disciplina $d$ alocadas na sala $s$ no turno $g$.
\item $fd_{a,d}$ - variável binária de folga para o aluno $a$ e disciplina demandada $d$.
\item $s_{a,d,g}$ - variável binária de alocação para o aluno $a$, disciplina demandada $d$, no turno $g$.
\item $a_{d,g,s}$ - variável inteira que indica o número de alunos atendidos na disciplina $d$, no turno $g$ e na sala $s$.
...
\forall g \in G \quad 
\forall s \in S \quad
\mbox{ aonde $alfa > 1$ é um fator de correção para forçar a criação de mais turmas.}
\end{eqnarray}
...
\subsubsection{Garante o mesmo número de turmas para disciplina prática e teórica, caso a relação seja 1x1}
\begin{eqnarray}
