...
\item $C_{d}$ - Total of credits of course $d$.
\item $M$ - big $M$.
...
\item $k_{p,i,d,hi,hf,t}$ - binary variable, indicates if professor $p$ teaches to section $i$ of course $d$ from time slot $hi$ to time slot $hf$ of day $t$. 
...
\sum\limits_{t \in T} \sum\limits_{i \in I_{d}} (fkp_{d,i,k} + fkm_{d,i,k})
      ...
      \\
      &
      + \sum\limits_{a \in A} \sum\limits_{t \in T} fagap_{a,t}
\end{array}
$$
...
\forall s \in S_{u} \quad
\forall t \in T \quad
\forall h \in H \quad (hi,hf)\text{ overlap }h
\end{eqnarray}


\subsubsection{Tries to satisfy students requirements}
\begin{eqnarray}
...
\begin{eqnarray}
\sum\limits_{s \in S}\sum\limits_{t \in T}\sum\limits_{hi \in H}\sum\limits_{hf \in H} NHC_{d,hi,hf} \cdot x_{i,d,s,t,hi,hf} = C_{d} \cdot z_{i,d,cp} \nonumber \qquad
\forall i \in I_{d} \quad
\forall d \in D \quad
\forall cp \in CP
\end{eqnarray}
...
\subsubsection{Avoids classes overlap at student timetable}
\begin{eqnarray}
...
\paragraph{Sets lower bound for variable $hip_{p,t,f}$}
\begin{eqnarray}
...
\paragraph{Sets upper bound for variable $hip_{p,t,f}$}
\begin{eqnarray}
...
\paragraph{Sets lower bound for variable $hfp_{p,t,f}$}
\begin{eqnarray}
...
\paragraph{Sets upper bound for variable $hfp_{p,t,f}$}
\begin{eqnarray}
...
\paragraph{Prohibits gap in phase of day \fixme{fase do dia} in professor timetable}
\begin{eqnarray}
...
\paragraph{Sets lower bound for variable $hia_{a,t}$}
\begin{eqnarray}
...
\paragraph{Sets upper bound for variable $hia_{a,t}$}
\begin{eqnarray}
...
\paragraph{Sets lower bound for variable $hfa_{a,t}$}
\begin{eqnarray}
...
\paragraph{Sets upper bound for variable $hfa_{a,t}$}
\begin{eqnarray}
...
\item $dpc_{p,d,c}$ - binary variable that indicates if professor $p$ teaches any lesson of course $d$ to major $c$.
\item $dpo_{p,d,o}$ - binary variable that indicates if professor $p$ teaches any lesson of course $d$ to offer $o$.
\item $fd_{a}$ - binary slack variable for assignment of lesson $a$.
...
\begin{array}{rl}
   \mbox{MIN} &
      ...
      \\                       
      &
       + \sum\limits_{p \in P} \sum\limits_{t \in T} ( hfp_{p,t} - hip_{p,t} )
      \\                                         
      &
       + 1000000 \cdot \sum\limits_{c \in A} classSize_{a} \cdot fd_{a}
      \\  
      &
...
\subsubsection{Sets if a professor teaches a course for a major (Sets $dpc_{p,c,d}$)}
\begin{eqnarray}
\sum\limits_{i \in I_{d,c} \cup I_{-d,c} } y_{p,d,i} \le nTurmas_{d} \cdot dpc_{p,c,d} \nonumber \qquad 
\forall p \in P \quad
\forall c \in C \quad
\forall d \in D_{theoretical}
\end{eqnarray}    
\begin{eqnarray}
...
\subsubsection{Ensures the maximum number of courses that a professor teaches for a major}
\begin{eqnarray}
...
\forall p \in P \quad
\forall t \in T \\
\forall \mbox{ (hf,t) e (hi,t+1) tal que } \Delta_{(hf,t), (hi,t+1)} < MinRestTime
\end{eqnarray}

\subsubsection{Sets if a professor teaches a course for a major's offer (Sets $dpo_{p,o,d}$)}
\begin{eqnarray}
...
\paragraph{Avoid gap in session of day in professor timetable}
\begin{eqnarray}
